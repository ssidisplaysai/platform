...
## Secret Scan Result
- Command evidence: .tmp-gcp-0002m1-r1a-secret-scan.txt
- Hit count: 154
- Classification:
...
- Command evidence: .tmp-gcp-0002m1-r1a-sensitive-scan.txt
- Hit count: 670
- Classification:
...
1. R1A remediated prior viewer read drift on collection APIs:
  - /api/sites
  - /api/products
  - /api/inventory
  now return 403 for viewer and 401 when unauthenticated.
2. Protected write/evaluate APIs continue to reject viewer role for audited samples.
3. Customer and profile APIs maintain read/write capability checks and now apply explicit organization/site read scope behavior.
...
- R1A closure: authorization inconsistency finding (F001) remediated.
- Remaining blocker outside this package: persistence transactional readiness finding (F002).
