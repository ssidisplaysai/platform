...
async function scoped(productSlug,stateSlug,citySlug){
  ...
  return {productId:pid,stateId:sid,items:Array.isArray(c)?c.map(x=>({id:x.id,slug:x.slug,status:x.status,parent:x.parent,link:x.link})):[]};
}
...
(async()=>{
  ...
  fs.writeFileSync('.tmp-newcity-4run-reliability.json', JSON.stringify(out,null,2));
  console.log(JSON.stringify({saved:'.tmp-newcity-4run-reliability.json',newCity,executionIds:out.newCityRuns.map(r=>r.executionId)},null,2));
})();