...
| `GLOBAL_HEADER_ALIGNMENT` | The certified global header used a generic wide row that did not align with the approved 1240px page composition and wrapped at intermediate widths. | Aligned the header row to 1240px, strengthened brand presence, kept desktop navigation on one line, and switched to the existing overlay menu at 1100px and below. | Desktop header 1425px wide, 71-169px high depending on wrapping; body grid started at 100px after composition repair. | Header row and body both 1240px at 1440; desktop header 119px; responsive header 89px; zero overflow. | Align global chrome to the primary composition grid and define an explicit navigation-collapse threshold before wrapping occurs. | High | Candidate for more cross-site examples; do not globalize yet. |
| `GLOBAL_CTA_HIERARCHY` | Request a Quote read as an ordinary navigation link despite being the approved primary conversion action. | Styled the existing final top-level navigation destination as a red 46px-minimum CTA while preserving its URL and mobile overlay placement. | Transparent 136px text link with no distinct action hierarchy. | Red/white 145px desktop CTA; same destination remains visible in the mobile overlay. | When navigation authority declares a primary conversion destination, give it distinct command styling without changing information architecture. | High | Candidate for more cross-site examples; do not globalize yet. |
