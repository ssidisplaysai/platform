...
(async()=>{
 ...
 fs.writeFileSync('.tmp-exec-60815.json',JSON.stringify(ex,null,2));
 const err=ex?.data?.resultData?.error||null;
 console.log(JSON.stringify({id,status:ex.status,lastNode:ex?.data?.resultData?.lastNodeExecuted||null,error:err?{message:err.message,description:err.description,node:err.node?.name,stack:err.stack?.split('\n').slice(0,5)}:null},null,2));
})();