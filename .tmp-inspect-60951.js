...
(async()=>{
 ...
 console.log(JSON.stringify({id,status:ex.status,lastNode:ex?.data?.resultData?.lastNodeExecuted,error:err?{message:err.message,description:err.description,node:err.node?.name}:null,callbackNodeOutput:cb},null,2));
})();