...
import {
  ...
  type EvidenceStructureKind,
  type EvidenceStructureNode,
...
const VALID_STRUCTURE_KINDS: readonly EvidenceStructureKind[] = ["flat", "hierarchical", "graph", "table"];
const VALID_LIFECYCLE_STATES: readonly EvidenceLifecycle[] = ["draft", "in-review", "approved", "frozen", "archived"];
...
function fail(code: EvidenceValidationErrorCode, message: string): never {
...
function isJsonValue(value: unknown): value is EvidenceJsonValue {
...
function assertJsonRecord(value: unknown, field: string): EvidenceJsonRecord {
  if (!isObject(value)) {
  ...
  return value as EvidenceJsonRecord;
}

function normalizeStringArray(values: readonly unknown[], field: string): readonly string[] {
...
function assertEncoding(value: unknown): EvidenceContent["encoding"] {
...
function assertLifecycle(value: unknown): EvidenceLifecycle {
  if (typeof value === "string" && VALID_LIFECYCLE_STATES.includes(value as EvidenceLifecycle)) {
    return value as EvidenceLifecycle;
  }

  fail("INVALID_VERSION", "version.lifecycle must be a supported lifecycle state");
}

function assertStructureKind(value: unknown): EvidenceStructureKind {
  if (typeof value === "string" && VALID_STRUCTURE_KINDS.includes(value as EvidenceStructureKind)) {
    return value as EvidenceStructureKind;
  }
...
function normalizeIdentity(input: EvidenceIdentity): EvidenceIdentity {
  return {
...
function normalizeSource(input: EvidenceSource): EvidenceSource {
...
function normalizeMetadata(input: EvidenceMetadata): EvidenceMetadata {
...
function normalizeContent(input: EvidenceContent): EvidenceContent {
...
function normalizeStructureNode(input: EvidenceStructureNode): EvidenceStructureNode {
  return {
...
function normalizeStructure(input: EvidenceStructure): EvidenceStructure {
...
function normalizeProvenance(input: EvidenceProvenance): EvidenceProvenance {
...
function normalizeRelationship(input: EvidenceRelationship): EvidenceRelationship {
  return {
...
function normalizeVersion(input: EvidenceVersion): EvidenceVersion {
  return {
...
function normalizeDraft(input: CanonicalEvidenceDraft): EvidencePayload {
...
function normalizeIntegrity(record: EvidencePayload, providedIntegrity?: Partial<EvidenceIntegrity>): EvidenceIntegrity {
  const checksum = sha256(record);
  const verifiedAt =
    providedIntegrity?.verifiedAt === undefined
...
function validateIntegrity(record: EvidencePayload, integrity: EvidenceIntegrity): void {
...
function validateRecord(record: CanonicalEvidenceRecord): void {
...
function cloneRecord(record: CanonicalEvidenceRecord): CanonicalEvidenceRecord {
  return JSON.parse(stableSerialize(record)) as CanonicalEvidenceRecord;
}

export class CanonicalEvidence {
  private readonly record: CanonicalEvidenceRecord;

  private constructor(record: CanonicalEvidenceRecord) {
  ...
  static create(input: CanonicalEvidenceDraft): CanonicalEvidence {
    ...
    const record: CanonicalEvidenceRecord = deepFreeze({ ...payload, integrity });
  ...
  static fromJSON(value: unknown): CanonicalEvidence {
    ...
    const relationships = assertArray(value.relationships, "relationships") as readonly EvidenceRelationship[];
    ...
    const payload = normalizeDraft({
      identity: value.identity as EvidenceIdentity,
      source: value.source as EvidenceSource,
      metadata: value.metadata as EvidenceMetadata,
      content: value.content as EvidenceContent,
      structure: value.structure as EvidenceStructure,
      provenance: value.provenance as EvidenceProvenance,
      relationships,
      version: value.version as EvidenceVersion,
    });

    const parsedIntegrity = integrity as unknown as EvidenceIntegrity;
    validateIntegrity(payload, parsedIntegrity);
    const record: CanonicalEvidenceRecord = deepFreeze({ ...payload, integrity: parsedIntegrity });
    validateRecord(record);
    return new CanonicalEvidence(record);
  }
...
export function createCanonicalEvidence(input: CanonicalEvidenceDraft): CanonicalEvidence {
  return CanonicalEvidence.create(input);
}

export function parseCanonicalEvidence(value: unknown): CanonicalEvidence {
  return CanonicalEvidence.fromJSON(value);
}

export function validateCanonicalEvidence(value: CanonicalEvidenceRecord): void {
  validateRecord(value);
}

export function computeCanonicalEvidenceChecksum(value: EvidencePayload): string {
  return sha256(value);
}

export function canonicalEvidencePayload(value: CanonicalEvidenceRecord): EvidencePayload {
  return {
