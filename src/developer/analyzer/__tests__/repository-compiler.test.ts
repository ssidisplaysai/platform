/**
 * repository-compiler.test.ts
 * Comprehensive tests for the Genesis Repository Compiler vertical slice.
 *
 * Tests cover:
 * - Recursive discovery
...
import { RepositoryCompiler } from '../RepositoryCompiler';

describe('Genesis Repository Compiler - Vertical Slice', () => {
  let tempDir: string;

  beforeAll(async () => {
    // Create temporary repository for testing
    tempDir = join(tmpdir(), `grc-test-${Date.now()}`);
  ...
  describe('RepositoryCompiler', () => {
    test('generates deterministic inventory for identical repositories', async () => {
      ...
      const report1 = await compiler.compile(repo1Dir);
      const report2 = await compiler.compile(repo2Dir);
    ...
    test('repeated compilations produce identical results', async () => {
      const compiler = new RepositoryCompiler();

      const testDir = join(tempDir, 'repeat-compile');
      await fs.mkdir(testDir, { recursive: true });
      await fs.writeFile(join(testDir, 'spec.md'), '# Spec v1.0\n\nReferences GCF-0001');

      const report1 = await compiler.compile(testDir);
      const report2 = await compiler.compile(testDir);
      const report3 = await compiler.compile(testDir);
    ...
    test('returns immutable compiled report', async () => {
      const compiler = new RepositoryCompiler();

      const testDir = join(tempDir, 'immutable-compiled');
      await fs.mkdir(testDir, { recursive: true });
      await fs.writeFile(join(testDir, 'doc.md'), 'Content');

      const report = await compiler.compile(testDir);
    ...
    test('is properly configured', () => {
      ...
      expect(compiler.isConfigured()).toBe(true);
      expect(compiler.getVersion()).toBe('1.0.0');
    });
  });

  describe('Determinism Contract', () => {
    test('identical repository produces identical results across repeated compilations', async () => {
      const compiler = new RepositoryCompiler();

      const testDir = join(tempDir, 'determinism-compile');
      await fs.mkdir(testDir, { recursive: true });
