...
export default async function SiteIntelligencePage({ params }: PageProps) {
  ...
  const publicBrandIdentity = brandProfile?.organizationId === site?.organizationId
    ...
    : site?.displayName ?? "";
  return <AppShell resourceSite={site ? createSiteContext(site) : null}>{site ? <SiteIntelligenceWorkspace organizationId={site.organizationId} siteId={site.siteId} siteName={site.displayName} publicBrandIdentity={publicBrandIdentity} /> : <div className="border border-zinc-800 p-6 text-zinc-300">Site not found.</div>}</AppShell>;
}