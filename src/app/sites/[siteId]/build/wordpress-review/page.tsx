...
import { SiteVisualPropagationAction } from "@/modules/foundation/SiteVisualPropagationAction";
...
export default async function SiteBuildWordPressReviewPage({ params }: { params: Promise<{ siteId: string }> }) {
  ...
  return <AppShell resourceSite={createSiteContext(site)}><div className="space-y-6"><SiteVisualPropagationAction site={{ organizationId: site.organizationId, siteId: site.siteId }} summary={workspace.remainingVisualSummary} /><SiteBuildWordPressDraftReviewWorkspace review={review} site={{ organizationId: site.organizationId, siteId: site.siteId, displayName: site.displayName }} /></div></AppShell>;
}