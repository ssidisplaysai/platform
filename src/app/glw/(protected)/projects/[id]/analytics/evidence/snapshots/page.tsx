...
export default async function ProjectAnalyticsEvidenceSnapshotsPage({ params }: PageProps) {
  const { id } = await params;
  if (!id || id.trim().length < 4) notFound();

  const permissions = await resolveAnalyticsPermissions("/glw/projects/[id]/analytics/evidence/snapshots");
  return (
