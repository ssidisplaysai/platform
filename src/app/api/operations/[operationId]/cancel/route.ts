...
import { cancelOperation, getOperationById } from "@/modules/foundation/operation-repository";
...
export async function POST(request: NextRequest, context: RouteContext) {
  const auth = authorizeRequest(request, "operations:cancel");
  if (!auth.ok) {
  ...
  const result = cancelOperation({ operationId, actor: actorFromRequest(request) });
