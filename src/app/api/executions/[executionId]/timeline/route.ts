...
import { getExecutionById, listExecutionTimeline } from "@/modules/foundation/execution-repository";

export async function GET(request: NextRequest, context: { params: Promise<{ executionId: string }> }) {
  const auth = authorizeRequest(request, "executions:view_timeline");
  if (!auth.ok) {
  ...
  return NextResponse.json({ timeline: listExecutionTimeline(executionId) });
}
