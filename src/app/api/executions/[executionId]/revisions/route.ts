...
import { getExecutionById, listExecutionRevisions } from "@/modules/foundation/execution-repository";

export async function GET(request: NextRequest, context: { params: Promise<{ executionId: string }> }) {
  const auth = authorizeRequest(request, "executions:view_revisions");
  if (!auth.ok) {
  ...
  return NextResponse.json({ revisions: listExecutionRevisions(executionId) });
}
