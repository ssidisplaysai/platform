...
import { getQuoteById, withdrawQuote } from "@/modules/foundation/quote-repository";

type RouteContext = { params: Promise<{ quoteId: string }> };

export async function POST(request: NextRequest, context: RouteContext) {
  const auth = authorizeRequest(request, "quotes:withdraw");
  if (!auth.ok) {
