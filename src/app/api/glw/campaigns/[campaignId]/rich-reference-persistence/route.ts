...
import { candidateMediaDataUrls, candidateVisualSummary, getIndianaRichReferenceCandidate } from "@/modules/glw/indiana-rich-reference-candidate";
...
export async function POST(request: NextRequest, context: Context) {
  try {
    ...
    if (body?.action === "CERTIFY") {
      ...
      const candidate = getIndianaRichReferenceCandidate(receipt.candidateId); if (!candidate || candidate.candidateSha !== receipt.candidateSha) throw new Error("INDIANA_CANDIDATE_AUTHORITY_STALE");
      const mediaAssignments = candidateMediaDataUrls(candidate).map((item) => ({ assignmentId: item.assignmentId, semanticRole: item.semanticRole, mediaId: item.mediaId, sourceUrl: item.dataUrl, contextId: candidate.mediaAuthorityFingerprint }));
