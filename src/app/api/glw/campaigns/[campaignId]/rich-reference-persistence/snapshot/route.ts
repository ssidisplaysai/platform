...
if (!PROFILE) throw new Error("INDIANA_SHARED_RICH_PAGE_PROFILE_REQUIRED");
...
export async function GET(request: NextRequest, context: { params: Promise<{ campaignId: string }> }) {
  ...
  $("title").text("Outdoor Digital Sphere in Indiana — WordPress draft review"); $("link[rel=canonical]").remove(); $("head").append(`<meta name="robots" content="noindex,nofollow"><style>${richPageHostIntegrationCss(PROFILE)}</style>`); $("iframe").remove();
