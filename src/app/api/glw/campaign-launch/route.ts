...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    ...
    promotion: {
      ...
      runningRelease: promotion.runningRelease,
      certifiedRelease: promotion.certifiedRelease,
    },
...
export async function POST(request: NextRequest) {
  ...
  const initialPromotion = readGlwCampaignLaunchPromotion();
  if (!initialPromotion.available) {
    return NextResponse.json({ error: initialPromotion.reason, code: "PRODUCTION_PROMOTION_REQUIRED", promotion: { state: initialPromotion.state } }, { status: 503 });
  }
  ...
  try {
    const mutationBoundaryPromotion = readGlwCampaignLaunchPromotion();
    if (!mutationBoundaryPromotion.available) {
      return NextResponse.json({ error: mutationBoundaryPromotion.reason, code: "PRODUCTION_PROMOTION_REQUIRED", promotion: { state: mutationBoundaryPromotion.state } }, { status: 503 });
    }
