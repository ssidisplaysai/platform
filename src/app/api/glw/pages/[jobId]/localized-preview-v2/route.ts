...
export async function POST(request: NextRequest, context: { params: Promise<{ jobId: string }> }) {
  ...
  if (scope.organizationId !== "ssi" || scope.siteId !== "site-ssi-projectorenclosure" || jobId !== "2ca74016-252b-4587-bf3c-ec9b7eb839c9" || body?.operation !== "CREATE_LOCALIZED_PREVIEW_V2" || Object.keys(body).some((key) => key !== "operation")) return NextResponse.json({ error: "Exact Dallas localized-preview operation required." }, { status: 400 });
  try { const bundle = await createDallasLocalizedPreviewV2({ actor: "GENESIS_LOCAL_CONTEXT_AND_PAGE_THEMING_V1" }); return NextResponse.json({ bundle, wordpressMutationPerformed: false, campaignMutationPerformed: false, publicationPerformed: false, dispatchPerformed: false }, { status: 201 }); }
  catch (error) { const message = error instanceof Error ? error.message : "LOCALIZED_PREVIEW_V2_FAILED"; return NextResponse.json({ error: message.split(":")[0], states: message.startsWith("DALLAS_LOCALIZED_MEDIA_GENERATION_FAILED:") ? message.split(":")[1]?.split(",").filter(Boolean) ?? [] : [], wordpressMutationPerformed: false, campaignMutationPerformed: false, publicationPerformed: false, dispatchPerformed: false }, { status: 409 }); }
}