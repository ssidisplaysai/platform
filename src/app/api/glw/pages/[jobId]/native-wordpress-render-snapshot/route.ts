...
export async function GET(request: NextRequest, context: { params: Promise<{ jobId: string }> }) {
  ...
  const current = await inspectSanAntonioWordPressDraft("13103"); if (current.identity.status !== "draft" || current.identity.template !== "elementor_header_footer" || current.featuredMediaId !== 10757 || current.hash !== receipt.afterHash || current.authority !== "POST_CONTENT") return new NextResponse("Repair identity stale", { status: 409 });
