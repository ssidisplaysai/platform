...
export async function POST(request: NextRequest) {
  try {
    ...
    const message = caught instanceof Error ? caught.message : "Unknown SEO refresh failure.";
    return NextResponse.json({ error: `SEO refresh internal failure: ${message}` }, { status: 500 });
  }
}
