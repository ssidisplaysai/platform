...
import { addCreativeInputs, ensureSiteIntelligenceWorkspace } from "@/modules/foundation/site-intelligence-repository";
...
export async function POST(request: NextRequest, context: Context) {
  ...
  const sentiment = String(form.get("sentiment") ?? "REFERENCE_ONLY") as CreativeInput["sentiment"];
  if (!SENTIMENTS.has(sentiment)) return NextResponse.json({ error: "Invalid asset preference." }, { status: 400 });
  ...
  try {
    ...
    workspace = addCreativeInputs({ organizationId: site.organizationId, siteId: site.siteId, expectedRevision: workspace.revision, actor: "site-owner", reason: "Owner uploaded classified creative inputs.", creativeInputs: assets.map((asset) => ({ inputId: asset.assetId, kind: asset.mediaType === "application/pdf" ? "TEXT" : "IMAGE", reference: asset.providerReference, sentiment, classification, notes: asset.note, suppliedBy: asset.uploadedBy, suppliedAt: asset.uploadedAt, binaryAsset: asset })) });
