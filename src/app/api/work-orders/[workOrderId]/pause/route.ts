...
export async function POST(request: NextRequest, context: RouteContext) {
  const auth = authorizeRequest(request, "work_orders:pause");
  if (!auth.ok) {
  ...
  const mutation = action === "resume" ? resumeWorkOrder : pauseWorkOrder;
  ...
  if (!result.workOrder) {
    return NextResponse.json({ error: "Unable to transition work order." }, { status: 400 });
  }
