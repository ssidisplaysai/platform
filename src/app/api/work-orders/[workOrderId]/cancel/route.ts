...
export async function POST(request: NextRequest, context: RouteContext) {
  const auth = authorizeRequest(request, "work_orders:cancel");
  if (!auth.ok) {
  ...
  if (!result.workOrder) {
    return NextResponse.json({ error: "Unable to cancel work order." }, { status: 400 });
  }
