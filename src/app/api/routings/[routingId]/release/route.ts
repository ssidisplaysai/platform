...
import { getRoutingById, releaseRouting } from "@/modules/foundation/routing-repository";
...
export async function POST(request: NextRequest, context: RouteContext) {
  const auth = authorizeRequest(request, "routings:release");
  if (!auth.ok) {
  ...
  const result = releaseRouting({ routingId, actor: actorFromRequest(request) });
