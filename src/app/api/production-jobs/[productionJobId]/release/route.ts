...
export async function POST(request: NextRequest, context: RouteContext) {
  const auth = authorizeRequest(request, "production_jobs:release");
  if (!auth.ok) {
  ...
  const result = releaseProductionJob({
    productionJobId,
  ...
  if (!result.productionJob) {
    return NextResponse.json({ error: "Unable to release production job." }, { status: 400 });
  }
