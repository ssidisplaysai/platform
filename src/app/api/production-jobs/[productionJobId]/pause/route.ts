...
export async function POST(request: NextRequest, context: RouteContext) {
  const auth = authorizeRequest(request, "production_jobs:pause");
  if (!auth.ok) {
  ...
  const mutation =
    action === "start" ? startProductionJob : action === "resume" ? resumeProductionJob : pauseProductionJob;
  ...
  if (!result.productionJob) {
    return NextResponse.json({ error: "Unable to transition production job." }, { status: 400 });
  }
