...
export function projectGlwReferenceWorkflow(job: GlwPageExecutionRecord | null): GlwReferenceWorkflowProjection {
  ...
  if (job.status === "FAILED") {
    ...
    return {
      state: job.errorCode === "GENERATED_CONTENT_QA_FAILED" && failures.length > 0
        ? "REFERENCE_BLOCKED"
        : "REFERENCE_GENERATION_FAILED",
      operationId: job.jobId,
      targetStateCode,
      ...
      artifactSha256: artifactSha256(job),
      qaFailures: failures,
      proposedRecoveryAction: job.errorCode === "GENERATED_CONTENT_QA_FAILED" && failures.length > 0
...
export function findEvidenceBoundLegacyReferenceJob(input: {
  ...
  if (owners.length !== 1 || owners[0].campaignId !== input.campaign.campaignId) return null;
