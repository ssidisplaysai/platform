...
describe("Outdoor Digital Sphere media authority route", () => {
  ...
  test("persists pending intake then a principal-bound approval without downstream mutations", async () => {
    ...
    const review = await POST(new NextRequest("http://localhost/api/glw/products/prod-outdoor-digital-sphere/media-authority", { method: "POST", headers: { ...commonHeaders, "content-type": "application/json" }, body: JSON.stringify({ action: "REVIEW_PRODUCT_MEDIA", mediaAuthorityId: pending.record.mediaAuthorityId, decision: "APPROVE", authorityClass: "PRODUCT_AUTHORITY", usageScopes: ["PRODUCT_AUTHORITY"], depictsActualProduct: true, heroEligible: true, altTextAuthority: "Outdoor Digital Sphere", captionAuthority: "Owner supplied.", authorityAndScopesConfirmed: true, localAtmosphereConfirmed: false }) }), context);
    expect(review.status).toBe(200);
    expect(await review.json()).toMatchObject({ record: { ownerApproval: "APPROVED", ownerApprovalTimestamp: expect.any(String), ownerPrincipalId: "robert", approvedUsageScopes: ["PRODUCT_AUTHORITY"], productRepresentationAllowed: true }, ownerApprovalPersisted: true, readiness: { heroAuthorityReady: true, supportingProductMediaReady: false, state: "PRODUCT_MEDIA_AUTHORITY_REQUIRED" }, n8nExecutionCreated: false, generationAttempted: false, wordpressMutation: false });
    const reload = await GET(new NextRequest("http://localhost/api/glw/products/prod-outdoor-digital-sphere/media-authority", { headers: { cookie, "x-gcp-organization-id": "led-display-warehouse", "x-gcp-site-id": "site-led-display-warehouse-production" } }), context);
    expect(reload.status).toBe(200);
    expect(await reload.json()).toMatchObject({ records: [{ mediaAuthorityId: pending.record.mediaAuthorityId, ownerApproval: "APPROVED", ownerPrincipalId: "robert", productRepresentationAllowed: true }] });
  });
});