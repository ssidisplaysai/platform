...
beforeEach(async () => { process.env.GCP_FOUNDATION_PERSISTENCE_DIR = `${process.cwd()}/.gcp-foundation-data-test-trusted-principal-${expect.getState().currentTestName?.replace(/\W+/g, "-")}`; process.env.GENESIS_OPERATOR_DIRECTORY_JSON = JSON.stringify([{ principalId: "operator-001", email: "operator@example.com", roles: ["platform_admin"], passwordHash: await createScryptPasswordHash(password, Buffer.alloc(16, 11)) }]); });
