...
export function deriveGeneratedPageReviewModel(input: {
  ...
  const applyState = input.job.jobId === "2ca74016-252b-4587-bf3c-ec9b7eb839c9" ? getDallasApplyState() : null; const applyReceipt=applyState?.receipts.at(-1)??null; const applyComparison=applyState?.comparisons?.at(-1)??null; const publicationState=input.job.jobId === "2ca74016-252b-4587-bf3c-ec9b7eb839c9"?getDallasPublicationState():null;const candidateReference=publicationState?.referenceCertifications.at(-1)??null;const reference=candidateReference&&!publicationState?.referenceRevocations.some((item)=>item.certificationId===candidateReference.certificationId)?candidateReference:null;
