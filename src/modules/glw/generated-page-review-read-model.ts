...
export function deriveGeneratedPageReviewModel(input: {
  ...
  const contrastReadiness = deriveGenesisContrastReadiness({ state: input.backgroundAwareContrastCertification?.state ?? null, authority: input.backgroundAwareContrastCertification?.identity.authority ?? null, hostRenderEvidenceAvailable: Boolean(input.wordpressStagingReceipt) });
