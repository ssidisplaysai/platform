...
export function deriveGeneratedPageReviewModel(input: {
  ...
  const actualNativeEvidence = input.job.jobId === SAN_ANTONIO_STAGING_JOB_ID ? SAN_ANTONIO_ACTUAL_NATIVE_CONTRAST_EVIDENCE : null; const historicalNativeMismatch = input.job.jobId === SAN_ANTONIO_STAGING_JOB_ID ? SAN_ANTONIO_NATIVE_PREVIEW_CONTRAST_MISMATCH_V1 : null; const nativeContradiction = historicalNativeMismatch && !actualNativeEvidence ? historicalNativeMismatch : null; const contrastReadiness = deriveGenesisContrastReadiness({ state: actualNativeEvidence?.state ?? input.backgroundAwareContrastCertification?.state ?? null, authority: actualNativeEvidence?.authority ?? input.backgroundAwareContrastCertification?.identity.authority ?? null, nativeHostEvidenceAvailable: Boolean(actualNativeEvidence ?? historicalNativeMismatch), higherAuthorityContradiction: Boolean(nativeContradiction) });
