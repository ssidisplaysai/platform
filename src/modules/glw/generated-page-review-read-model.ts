...
export function deriveGeneratedPageReviewModel(input: {
  ...
  const nativeContradiction = input.job.jobId === SAN_ANTONIO_STAGING_JOB_ID ? SAN_ANTONIO_NATIVE_PREVIEW_CONTRAST_MISMATCH_V1 : null; const contrastReadiness = deriveGenesisContrastReadiness({ state: input.backgroundAwareContrastCertification?.state ?? null, authority: input.backgroundAwareContrastCertification?.identity.authority ?? null, nativeHostEvidenceAvailable: Boolean(nativeContradiction), higherAuthorityContradiction: Boolean(nativeContradiction) });
  ...
  return {
    ...
    backgroundAwareContrast: input.backgroundAwareContrastCertification ? { contract: input.backgroundAwareContrastCertification.contract, certificationId: input.backgroundAwareContrastCertification.certificationId, state: input.backgroundAwareContrastCertification.state, failureCount: input.backgroundAwareContrastCertification.failureCount, ownerReviewReady: contrastReadiness.ownerReviewReady, publicationReady: contrastReadiness.publicationReady, authority: input.backgroundAwareContrastCertification.identity.authority, generatedImageReevaluationPerformed: input.backgroundAwareContrastCertification.generatedImageReevaluationPerformed, viewports: input.backgroundAwareContrastCertification.captures.map((capture) => ({ viewport: capture.viewport, observations: capture.results.length, failures: capture.failures })) } : null,
    durableReviewDecisionExists: false,
  };
}
