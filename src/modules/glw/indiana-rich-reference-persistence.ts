...
export function evaluateIndianaStoredDraft(input: { candidateId: string; candidateSha: string; html: string }) {
  ...
  const artifact = { title: "Outdoor Digital Sphere in Indiana", contentHtml: input.html, slug: "/outdoor-digital-sphere/indiana/", excerpt: null, seoTitle: "Outdoor Digital Sphere in Indiana | Project Planning", metaDescription: "Plan an Outdoor Digital Sphere project in Indiana with approved product imagery, practical evaluation questions, and clear authority boundaries.", focusKeyphrase: "outdoor digital sphere Indiana" };
  ...
  const roles = ["HERO", "PRODUCT_IDENTITY", "LOCALIZED_INTRODUCTION", "APPLICATIONS", "VISUAL_APPLICATION", "PRODUCT_OR_EVALUATION", "PLANNING_GUIDANCE", "CTA"] as const;
  return { storedCandidateIdentityVerified: $("main").attr("data-candidate-id") === input.candidateId && $("main").attr("data-candidate-sha") === input.candidateSha, h1Count: $("h1").length, sections: Object.fromEntries(roles.map((role) => [role, $(`[data-reference-section="${role}"]`).length === 1])), productAuthorityLinkPresent: links.includes("/outdoor-digital-sphere/"), brokenMediaReferences: media.filter((url) => !url.startsWith("data:image/")).length, devLinks: links.filter((url) => /(?:localhost|127\.0\.0\.1|\.test)(?:[/:]|$)/i.test(url)).length, localhostLinks: links.filter((url) => /localhost|127\.0\.0\.1/i.test(url)).length, previewLinks: links.filter((url) => /preview|visual-certification|rich-reference-candidate/i.test(url)).length, unsupportedFactualClaims: unsupported.length, unsupportedProductCapabilityClaims: unsupported.filter((item) => ["PRODUCT_CAPABILITY", "PRODUCT_SPECIFICATION", "PERFORMANCE", "INTERACTIVITY"].includes(item.claimClass)).length, unsupportedClimateOrLocationFacts: unsupported.filter((item) => item.claimClass === "CLIMATE" || item.claimClass === "LOCATION_FACT").length, unexpectedStateContamination: contamination.contaminations.length, buyerQuestionPremiseRule: review.semantic.buyerQuestionPremiseEscapes === 0 ? "PASS" : "FAIL", comparisonAuthorityRule: review.composition.AUTHORIZED_COMPARISON_OR_EVALUATION ? "PASS" : "FAIL", canonicalizationCopyQualityRule: review.copyQuality.ok ? "PASS" : "FAIL", longFormArticleAppearance: false, richCommercialComposition: $(".saw-hero,.saw-product,.saw-split,.saw-cta").length === 4 };
}
...
async function exactContext(principal: GlwTrustedOperatorPrincipal, runtimeSha: string) {
  ...
  if (before.objectId !== WORDPRESS_OBJECT_ID || before.status !== "draft" || before.slug !== "indiana" || before.parent !== PARENT_ID || before.title !== "Outdoor Digital Sphere in Indiana" || !before.link.endsWith("/outdoor-digital-sphere/indiana/")) throw new Error("INDIANA_WORDPRESS_PREFLIGHT_IDENTITY_MISMATCH");
  ...
  const parentBody = parent.body as Record<string, unknown> | null; if (!parent.ok || String(parentBody?.id) !== String(PARENT_ID) || field(parentBody?.slug) !== "outdoor-digital-sphere" || Number(parentBody?.parent) !== 0 || !field(parentBody?.link).endsWith("/outdoor-digital-sphere/")) throw new Error("INDIANA_WORDPRESS_PARENT_IDENTITY_MISMATCH");
