...
const previewCopyByProductId: Record<string, ProductPreviewCopy> = {
  "prod-outdoor-digital-sphere": {
    productLabel: "Outdoor Digital Sphere",
    productCategory: "Immersive LED sphere systems",
    productDescription:
      "Immersive spherical LED presentation engineered for high-visibility outdoor venues, destinations, and shared public experiences.",
    fallbackAltText: "Outdoor digital sphere display",
  },
};

const defaultPreviewCopy: ProductPreviewCopy = {
  productLabel: "Product Authority",
  productCategory: "Owner-approved product system",
  productDescription:
    "Owner-approved product authority media and governed composition evidence for this page identity.",
  fallbackAltText: "Owner-approved product authority image",
};
...
export function GlwRichCompositionPreview({ preview, compact = false }: { preview: Preview; compact?: boolean }) {
  ...
  return <article className="overflow-hidden bg-[#f4f3ef] text-[#172022]" data-preview-authority="NON_MUTATING" data-composition-profile="LOCATION_SERVICE">
    ...
    <header className="flex items-center justify-between border-b border-[#d7d4cc] bg-white px-5 py-4 sm:px-8"><span className="text-sm font-black uppercase tracking-[0.16em] text-[#172022]">{copy.productLabel}</span><span className="text-xs font-semibold uppercase text-[#66706f]">{copy.productCategory}</span></header>
    ...
    <section className={`grid items-center gap-8 border-b border-[#d7d4cc] bg-white px-5 py-12 sm:px-8 ${compact ? "lg:grid-cols-2" : "lg:grid-cols-[1.08fr_.92fr] lg:px-[8vw] lg:py-20"}`} data-media-role="PRODUCT_AUTHORITY">
      <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#b42318]">Approved Product Authority</p><h2 className="mt-3 text-3xl font-black leading-tight text-[#172022] sm:text-4xl">{preview.productName}</h2><p className="mt-4 max-w-xl text-base leading-7 text-[#505b5b]">{copy.productDescription}</p><p className="mt-5 text-xs font-semibold uppercase text-[#66706f]">Assigned for preview · Not rendered in current WordPress</p></div>
