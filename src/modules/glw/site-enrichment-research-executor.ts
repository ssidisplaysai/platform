...
export type GlwResearchProvider = {
  research(
    input: GlwResearchExecutionRequest,
  ): Promise<GlwResearchAcquisition>;
};
...
function assertExecutionIdentity(
  ...
  if (
    ...
    throw new Error(
      "Research execution identity does not match the persisted enrichment work item.",
    );
  }
}

function assertAcquisitionIdentity(
  request: GlwResearchExecutionRequest,
  ...
  if (
    ...
    || acquisition.productId
      !== request.productId
    || acquisition.stateCode
      !== request.stateCode
        .trim()
        .toUpperCase()
    || acquisition.canonicalPath
      !== request.canonicalPath
    || acquisition.jobId
      !== request.jobId
    || acquisition.wordpressObjectId
      !== request.wordpressObjectId
  ) {
    throw new Error(
      "Research provider returned evidence for a different page identity.",
    );
  }
}
...
function assertUniqueIds(
  ...
  if (unique.size !== values.length) {
    throw new Error(
      `Research provider returned duplicate ${label} identifiers.`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Research provider returned an empty ${label} identifier.`,
    );
  }
}
...
function assertSources(
  sources: readonly GlwEnrichmentSource[],
): Map<string, GlwEnrichmentSource> {
  assertUniqueIds(
    sources.map(
  ...
  for (const source of sources) {
    ...
    if (
      ...
      throw new Error(
        `Research source ${source.sourceId} has an invalid HTTPS URL/domain identity.`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Research source ${source.sourceId} is missing required provenance.`,
      );
    }
...
function assertClaims(
  claims: readonly GlwEnrichmentClaim[],
  sourceMap:
    ReadonlyMap<string, GlwEnrichmentSource>,
): void {
  assertUniqueIds(
    claims.map(
      (claim) => claim.claimId,
  ...
  for (const claim of claims) {
    if (
      ...
      throw new Error(
        `Research claim ${claim.claimId} has no evidence.`,
      );
    }

    for (
      const sourceId
      of claim.evidenceSourceIds
    ) {
      if (!sourceMap.has(sourceId)) {
        throw new Error(
          `Research claim ${claim.claimId} references unknown evidence ${sourceId}.`,
        );
      }
    }
  }
...
function assertLinks(
  ...
  const linkMap =
    new Map<string, GlwEnrichmentLink>();
  ...
  for (const link of links) {
    if (
      ...
      throw new Error(
        `Research link ${link.linkId} is incomplete.`,
      );
    }
    ...
    if (link.kind === "internal") {
      if (
        ...
        throw new Error(
          `Internal research link ${link.linkId} must be site-relative.`,
        );
      }
    } else {
      if (
        ...
        throw new Error(
          `External research link ${link.linkId} must use HTTPS.`,
        );
      }
      ...
      if (
        ...
        throw new Error(
          `External research link ${link.linkId} must resolve to stored evidence.`,
        );
      }
    }
...
function sourceHasTier(
  sourceId: string,
  requiredTier:
    GlwEnrichmentSourceTier | null | undefined,
  sourceMap:
    ReadonlyMap<string, GlwEnrichmentSource>,
): boolean {
  if (!requiredTier) {
...
function fulfillRequirements(
  ...
  for (
    ...
    if (
      ...
      throw new Error(
        `Research provider attempted to fulfill unknown requirement ${requirementId}.`,
      );
    }
  }
  ...
  return input.requirements.map(
    (requirement) => {
      ...
      if (
        ...
        for (
          ...
          if (
            !sourceHasTier(
              sourceId,
              requirement.sourceTier,
              input.sourceMap,
            )
          ) {
            throw new Error(
              `Research requirement ${requirement.requirementId} was fulfilled with an invalid source tier.`,
            );
          }
        }
      ...
      for (
        ...
        const link =
          input.linkMap.get(linkId);

        if (
          !link
          || !linkMatchesRequirement(
            requirement,
            link,
          ...
          throw new Error(
            `Research requirement ${requirement.requirementId} was fulfilled with an invalid link.`,
          );
        }
      }
...
export async function executeGlwSiteEnrichmentResearch(
  input: {
    request:
  ...
  if (!record) {
    throw new Error(
      "Research work item was not found.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Research execution requires research_pending status; found ${record.status}.`,
    );
  }
  ...
  const acquisition =
    await input.provider.research(
      input.request,
    );

  assertAcquisitionIdentity(
    input.request,
    acquisition,
  );
