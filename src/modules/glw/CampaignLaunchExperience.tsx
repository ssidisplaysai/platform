...
export function CampaignStatusPanel({ counts }: { counts?: Partial<GlwLaunchStatusCounts> }) {
  const values: GlwLaunchStatusCounts = { researching: 0, generating: 0, qa: 0, draft: 0, review: 0, published: 0, failed: 0, ...counts };
  return (
    <section aria-label="Campaign status" className="border-t border-zinc-700 pt-5">
      ...
      <div className="mt-3 grid grid-cols-2 gap-px overflow-hidden bg-zinc-800 sm:grid-cols-4 lg:grid-cols-7">
        {Object.entries(values).map(([label, count]) => <div key={label} className="bg-zinc-950 p-3"><p className="text-xs capitalize text-zinc-500">{label}</p><p className="mt-1 text-lg font-semibold text-white">{count}</p></div>)}
      </div>
    </section>
  );
}
...
export function CampaignLaunchOutcome({ result, productName, reach, onAnalyzeAgain, onStartAnother }: {
  ...
  return (
    <section className={`space-y-5 border p-5 ${attention ? "border-amber-500/50 bg-amber-950/20" : "border-emerald-500/50 bg-emerald-950/20"}`}>
      ...
      {result.campaignId ? <CampaignStatusPanel counts={result.state === "DISPATCH_STARTED" ? { researching: result.selectedTargetCount } : result.state === "REFERENCE_REVIEW_REQUIRED" ? { review: 1 } : result.state === "RECOVERY_REQUIRED" ? { failed: 1 } : { draft: result.selectedTargetCount }} /> : null}
