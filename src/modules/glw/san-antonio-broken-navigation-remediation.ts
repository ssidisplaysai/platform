...
export function applySanAntonioBrokenNavigationRemediation(input: string) {
  ...
  for (const item of SAN_ANTONIO_LINK_REMEDIATIONS) { const escaped = item.oldHref.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); const href = new RegExp(`href=(["'])(?:https?:\\/\\/(?:www\\.)?projectorenclosure\\.com)?${escaped}\\1`, "gi"); const matches = [...output.matchAll(href)]; if (matches.length !== 1) throw new Error(`SAN_ANTONIO_BROKEN_LINK_CARDINALITY_${item.oldHref.replace(/\W+/g, "_").toUpperCase()}:${matches.length}`); output = item.newHref ? output.replace(href, (_match, quote: string) => `href=${quote}${item.newHref}${quote}`) : output.replace(new RegExp(`<a\\b([^>]*?)href=(["'])(?:https?:\\/\\/(?:www\\.)?projectorenclosure\\.com)?${escaped}\\2([^>]*)>([\\s\\S]*?)<\\/a>`, "i"), (_match, before: string, _quote: string, after: string, content: string) => `<span data-genesis-removed-dead-link="${item.oldHref}"${before}${after}>${content}</span>`); }
  if (SAN_ANTONIO_LINK_REMEDIATIONS.some((item) => new RegExp(`href=["'](?:https?:\\/\\/(?:www\\.)?projectorenclosure\\.com)?${item.oldHref.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}["']`, "i").test(output))) throw new Error("SAN_ANTONIO_BROKEN_NAVIGATION_TRANSFORM_INCOMPLETE"); return output;
}
