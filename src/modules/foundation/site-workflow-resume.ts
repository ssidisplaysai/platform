...
export function resolveSiteWorkflowResume(input: { site: SiteConfiguration; intelligence: SiteIntelligenceWorkspace | null; productAuthority: ProductAuthorityProgress; generationReadiness: { readyToCertify: boolean; certified: boolean; stale: boolean; blockers: string[] }; siteBuildStarted?: boolean; siteBuildStage?: string }): SiteWorkflowResume {
  ...
  else primaryAction = { key: "CONTINUE_SITE_BUILD", title: "Site Build", description: input.siteBuildStage === "WORDPRESS_DRAFT_REVIEW" ? "The exact WordPress drafts are synchronized. Continue to draft review and site QA; publication remains disabled." : input.siteBuildStage === "WORDPRESS_CONTENT_UPDATE" ? "Page review is complete. Continue to update the exact existing WordPress drafts; nothing will be published." : "Generation Readiness is certified. Starting the bounded build remains a separate explicit owner action.", label: input.siteBuildStarted ? "CONTINUE SITE BUILD" : "START SITE BUILD", href: scoped(`/sites/${site.siteId}/build`, site) };
