...
export function resolveSiteWorkflowResume(input: { site: SiteConfiguration; intelligence: SiteIntelligenceWorkspace | null; productAuthority: ProductAuthorityProgress; generationReadiness: { readyToCertify: boolean; certified: boolean; stale: boolean; blockers: string[] }; siteBuildStarted?: boolean }): SiteWorkflowResume {
  ...
  const stages: SiteWorkflowStage[] = [
    ...
    { key: "site_build", label: "Site Build", status: input.siteBuildStarted ? "IN_PROGRESS" : "NOT_STARTED", detail: input.siteBuildStarted ? "A bounded Site Build session has been explicitly started." : "No Site Build has been started from this workflow." },
  ...
  else primaryAction = { key: "CONTINUE_SITE_BUILD", title: "Site Build", description: "Generation Readiness is certified. Starting the bounded build remains a separate explicit owner action.", label: input.siteBuildStarted ? "CONTINUE SITE BUILD" : "START SITE BUILD", href: scoped(`/sites/${site.siteId}/build`, site) };
