...
export function resolveSiteWorkflowResume(input: { site: SiteConfiguration; intelligence: SiteIntelligenceWorkspace | null; productAuthority: ProductAuthorityProgress; generationReadiness: { readyToCertify: boolean; certified: boolean; stale: boolean; blockers: string[] }; siteBuildStarted?: boolean; siteBuildStage?: string }): SiteWorkflowResume {
  ...
  const stages: SiteWorkflowStage[] = [
    ...
    { key: "site_build", label: "Site Build", status: input.siteBuildStarted ? "IN_PROGRESS" : "NOT_STARTED", detail: input.siteBuildStage === "WORDPRESS_CONTENT_UPDATE" ? "Page review is complete. Continue to the explicit WordPress draft content update." : input.siteBuildStarted ? "A bounded Site Build session has been explicitly started." : "No Site Build has been started from this workflow." },
