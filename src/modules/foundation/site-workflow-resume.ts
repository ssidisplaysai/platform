...
export type SiteResumeAction = { key: "CONTINUE_SITE_ONBOARDING" | "CONTINUE_SITE_INTELLIGENCE" | "CONTINUE_CAPABILITY_REVIEW" | "REVIEW_STRATEGY" | "CONTINUE_CREATIVE_DIRECTION" | "CONTINUE_PRODUCT_SERVICE_AUTHORITY" | "CONTINUE_GENERATION_READINESS" | "CONTINUE_SITE_BUILD" | "REVIEW_REMAINING_DESIGNS" | "REVIEW_SITE_QA" | "REVIEW_NAVIGATION" | "REVIEW_PUBLICATION_READINESS" | "AUTHORIZE_PUBLICATION" | "REVIEW_PUBLICATION_EXECUTION" | "RESUME_PUBLICATION_EXECUTION" | "VERIFY_PUBLICATION" | "REVIEW_COMPLETED_SITE" | "SHOW_PUBLICATION_BLOCKERS"; title: string; description: string; label: string; href: string };
...
export function resolveSiteWorkflowResume(input: { site: SiteConfiguration; intelligence: SiteIntelligenceWorkspace | null; productAuthority: ProductAuthorityProgress; generationReadiness: { readyToCertify: boolean; certified: boolean; stale: boolean; blockers: string[] }; siteBuildStarted?: boolean; siteBuildStage?: string; siteBuildNext?: { action: string; label: string; detail: string; route: string }; continuationProjection?: ThreeGateOwnerActionProjection }): SiteWorkflowResume {
  ...
  const generationBlockers = generationReadiness.readyToCertify ? [] : generationReadiness.blockers.length ? generationReadiness.blockers : ["Generation readiness requirements are not complete."];
  ...
  const stages: SiteWorkflowStage[] = [
    ...
    { key: "generation_readiness", label: "Generation Readiness", status: productComplete ? generationReadiness.certified ? "COMPLETE" : generationReadiness.readyToCertify ? "READY_FOR_REVIEW" : "BLOCKED" : "BLOCKED", detail: productComplete ? generationReadiness.certified ? "The current authority snapshot is explicitly certified for safe draft generation." : generationReadiness.stale ? "Upstream authority changed; review and recertify Generation Readiness." : generationReadiness.readyToCertify ? "Final checks pass; explicit owner certification is required." : "Review the remaining draft-generation requirements." : "Complete Product / Service Authority first." },
    { key: "site_build", label: "Site Build", status: input.siteBuildStarted ? "IN_PROGRESS" : "NOT_STARTED", detail: input.siteBuildStage === "WORDPRESS_DRAFT_REVIEW" ? "WordPress content synchronization is complete. Review the exact drafts and site QA." : input.siteBuildStage === "WORDPRESS_CONTENT_UPDATE" ? "Page review is complete. Continue to the explicit WordPress draft content update." : input.siteBuildStarted ? "A bounded Site Build session has been explicitly started." : "No Site Build has been started from this workflow." },
  ...
  else if (input.siteBuildStarted && input.siteBuildNext?.action === "AUTHORIZE_PUBLICATION") primaryAction = { key: "AUTHORIZE_PUBLICATION", title: "Publication Authorization", description: input.siteBuildNext.detail, label: input.siteBuildNext.label, href: input.siteBuildNext.route };
  else if (input.siteBuildStarted && input.siteBuildNext?.action === "REVIEW_PUBLICATION_EXECUTION") primaryAction = { key: "REVIEW_PUBLICATION_EXECUTION", title: "Publication Execution", description: input.siteBuildNext.detail, label: input.siteBuildNext.label, href: input.siteBuildNext.route };
