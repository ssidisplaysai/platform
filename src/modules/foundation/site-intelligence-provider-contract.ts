...
const CONTINUATION_RESPONSE_LIMITS = { minEvidence: 4, maxEvidence: 10, maxOpportunities: 1 } as const;
...
export async function parseSiteIntelligenceProviderResponseV1(value: unknown, expected: SiteIntelligenceProviderRequestV1, dependencies: ProviderContractDependencies = {}): Promise<SiteIntelligenceProviderResponseV1> {
  const continuationMode = expected.executionMode === "OPPORTUNITY_CONTINUATION";
  ...
  const minEvidence = continuationMode ? CONTINUATION_RESPONSE_LIMITS.minEvidence : 1;
  const maxEvidence = continuationMode ? CONTINUATION_RESPONSE_LIMITS.maxEvidence : SITE_INTELLIGENCE_PROVIDER_LIMITS.maxEvidence;
  if (!Array.isArray(root.evidence) || root.evidence.length < minEvidence || root.evidence.length > maxEvidence) fail("RESEARCH_PROVIDER_RESPONSE_INVALID", "evidence_bounds");
  ...
  const maxOpportunities = continuationMode ? CONTINUATION_RESPONSE_LIMITS.maxOpportunities : SITE_INTELLIGENCE_PROVIDER_LIMITS.maxOpportunities;
  if (!Array.isArray(root.opportunities) || root.opportunities.length === 0 || root.opportunities.length > maxOpportunities) fail("RESEARCH_PROVIDER_RESPONSE_INVALID", "opportunity_bounds");
  ...
  if (continuationMode && expected.focusOpportunityId && opportunities[0]?.opportunityId !== expected.focusOpportunityId) fail("RESEARCH_PROVIDER_CONTRACT_MISMATCH", "continuation_focus_mismatch");
