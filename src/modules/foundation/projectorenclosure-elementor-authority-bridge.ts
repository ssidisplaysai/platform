...
export async function readProjectorEnclosureElementorAuthority(pair: typeof HEADER_AUTHORITY | typeof FOOTER_AUTHORITY) { return bridgeRead(authority(), pair); }
export async function updateProjectorEnclosureElementorAuthority(input: { pair: typeof HEADER_AUTHORITY | typeof FOOTER_AUTHORITY; expectedDocumentHash: string; expectedWidgetHash: string; replacementWidgetHTML: string }) { const resolved = authority(); const response = await fetch(`${resolved.origin}/wp-json${BRIDGE_PATH}/${input.pair.documentId}/${input.pair.widgetId}`, { method: "POST", headers: { ...resolved.headers, "Content-Type": "application/json" }, body: JSON.stringify({ expectedDocumentHash: input.expectedDocumentHash, expectedWidgetHash: input.expectedWidgetHash, replacementWidgetHTML: input.replacementWidgetHTML }), cache: "no-store", signal: AbortSignal.timeout(30_000) }); const body = await response.json().catch(() => null) as { backupId?: string; authority?: ElementorBridgeAuthority; code?: string } | null; if (!response.ok || !body?.backupId || !body.authority) throw new Error(`ELEMENTOR_BRIDGE_AUTHORITY_UPDATE_FAILED:${input.pair.documentId}:${response.status}:${body?.code ?? "UNKNOWN"}`); return { backupId: body.backupId, authority: body.authority }; }
export async function rollbackProjectorEnclosureElementorAuthority(input: { pair: typeof HEADER_AUTHORITY | typeof FOOTER_AUTHORITY; expectedCurrentDocumentHash: string; backupId: string }) { const resolved = authority(); const response = await fetch(`${resolved.origin}/wp-json${BRIDGE_PATH}/${input.pair.documentId}/${input.pair.widgetId}/rollback`, { method: "POST", headers: { ...resolved.headers, "Content-Type": "application/json" }, body: JSON.stringify({ expectedCurrentDocumentHash: input.expectedCurrentDocumentHash, backupId: input.backupId }), cache: "no-store", signal: AbortSignal.timeout(30_000) }); const body = await response.json().catch(() => null) as { rolledBack?: boolean; authority?: ElementorBridgeAuthority; code?: string } | null; if (!response.ok || body?.rolledBack !== true || !body.authority) throw new Error(`ELEMENTOR_BRIDGE_AUTHORITY_ROLLBACK_FAILED:${input.pair.documentId}:${response.status}:${body?.code ?? "UNKNOWN"}`); return body.authority; }
export async function installProjectorEnclosureElementorAuthorityBridge() { const preflight = await preflightProjectorEnclosureElementorBridgeAuthority(); if (!preflight.authenticated || !preflight.manageOptionsCapability || !preflight.codeSnippetsAdministrationAuthority) throw new Error(`ELEMENTOR_BRIDGE_AUTHORITY_PREFLIGHT_FAILED:${preflight.codeSnippetsHttpStatus}`); const resolved = authority(); const matches = (await listSnippets(resolved.origin, resolved.headers)).filter((item) => item.name === SNIPPET_NAME); if (matches.length > 1) throw new Error("ELEMENTOR_BRIDGE_SNIPPET_COLLISION"); let snippet = matches[0] ?? null; let created = false; if (!snippet) { const response = await fetch(`${resolved.origin}/wp-json/code-snippets/v1/snippets`, { method: "POST", headers: { ...resolved.headers, "Content-Type": "application/json" }, body: JSON.stringify({ name: SNIPPET_NAME, desc: "Temporary exact-pair Elementor HTML widget authority bridge for ProjectorEnclosure global navigation repair.", code: PROJECTORENCLOSURE_ELEMENTOR_AUTHORITY_BRIDGE_CODE, tags: ["genesis", "elementor", "bounded-authority"], scope: "global", active: true, priority: 10 }), cache: "no-store", signal: AbortSignal.timeout(30_000) }); const createdSnippet = await response.json().catch(() => null) as Snippet | null; if (!response.ok || !createdSnippet?.id) throw new Error(`ELEMENTOR_BRIDGE_SNIPPET_CREATE_FAILED:${response.status}`); snippet = createdSnippet; created = true; } else if (snippet.code !== PROJECTORENCLOSURE_ELEMENTOR_AUTHORITY_BRIDGE_CODE || snippet.scope !== "global") throw new Error("ELEMENTOR_BRIDGE_SNIPPET_DRIFT"); if (!snippet) throw new Error("ELEMENTOR_BRIDGE_SNIPPET_REQUIRED"); if (!snippet.active) { const response = await fetch(`${resolved.origin}/wp-json/code-snippets/v1/snippets/${snippet.id}/activate`, { method: "POST", headers: resolved.headers, cache: "no-store", signal: AbortSignal.timeout(30_000) }); if (!response.ok) throw new Error(`ELEMENTOR_BRIDGE_ACTIVATION_FAILED:${response.status}`); }
