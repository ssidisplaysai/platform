...
export function deriveRenderedVisualFindings(input: { layoutClass: RenderedVisualLayoutClass; captures: readonly RenderedVisualCaptureEvidence[] }): RenderedVisualFinding[] {
  ...
  for (const capture of input.captures) {
    ...
    findings.push(finding({ findingCode: `${capture.viewportClass}_PLANNING_GRID_BALANCE`, category: "LAYOUT", state: capture.hostIntegration.planningGridBalanced ? "PASS" : "BLOCKED", summary: capture.hostIntegration.planningGridBalanced ? "The seven-item planning grid is intentionally balanced." : "The planning grid leaves an unbalanced orphan position.", evidenceReferences: [capture.captureId], rule: { ruleId: "RVC_PLANNING_GRID_BALANCE" }, safeRecommendation: capture.hostIntegration.planningGridBalanced ? null : "Center or span the final planning row without adding synthetic content." }));
