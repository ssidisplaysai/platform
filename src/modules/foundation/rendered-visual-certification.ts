...
export type RenderedVisualCaptureEvidence = {
  ...
  hostIntegration: {
    ...
    ctaHeadingCollision: boolean;
    ctaCopyCollision: boolean;
    ctaButtonCollision: boolean;
    buttonContainerOverflow: boolean;
    unexpectedElementCollision: boolean;
    stickyHeaderUnexpectedOcclusion: boolean;
    planningGridPresent?: boolean;
    planningGridBalanced: boolean;
...
export function validateRenderedVisualCertification(certification: RenderedVisualCertification): RenderedVisualCertification {
  ...
  for (const capture of certification.captures) {
    ...
    bounds(capture.hero.bounds, "VISUAL_HERO_BOUNDS_INVALID"); bounds(capture.hero.headingBounds, "VISUAL_HEADING_BOUNDS_INVALID"); bounds(capture.hero.primaryCtaBounds, "VISUAL_CTA_BOUNDS_INVALID"); bounds(capture.hero.mediaBounds, "VISUAL_HERO_MEDIA_BOUNDS_INVALID"); finite(capture.hostIntegration.blankImageContainers, "VISUAL_BLANK_IMAGE_COUNT_INVALID");
...
export function deriveRenderedVisualFindings(input: { layoutClass: RenderedVisualLayoutClass; captures: readonly RenderedVisualCaptureEvidence[] }): RenderedVisualFinding[] {
  ...
  for (const capture of input.captures) {
    findings.push(finding({ findingCode: `${capture.viewportClass}_HOST_HEADER_OVERLAP`, category: "LAYOUT", state: capture.hostIntegration.headerOverlap ? "BLOCKED" : "PASS", summary: capture.hostIntegration.headerOverlap ? "The host header overlaps primary content." : "The host header does not overlap primary content.", evidenceReferences: [capture.captureId], rule: { ruleId: "RVC_HOST_HEADER_OVERLAP" }, safeRecommendation: capture.hostIntegration.headerOverlap ? "Repair the reusable host integration before approval." : null }));
    findings.push(finding({ findingCode: `${capture.viewportClass}_HOST_FOOTER_OVERLAP`, category: "LAYOUT", state: capture.hostIntegration.footerOverlap ? "BLOCKED" : "PASS", summary: capture.hostIntegration.footerOverlap ? "The host footer overlaps primary content." : "The host footer does not overlap primary content.", evidenceReferences: [capture.captureId], rule: { ruleId: "RVC_HOST_FOOTER_OVERLAP" }, safeRecommendation: capture.hostIntegration.footerOverlap ? "Repair the reusable host integration before approval." : null }));
    findings.push(finding({ findingCode: `${capture.viewportClass}_BLANK_IMAGE_CONTAINERS`, category: "MEDIA", state: capture.hostIntegration.blankImageContainers > 0 ? "BLOCKED" : "PASS", summary: capture.hostIntegration.blankImageContainers > 0 ? `${capture.hostIntegration.blankImageContainers} visible image containers have no decoded image content.` : "All visible image containers contain decoded image content.", evidenceReferences: [capture.captureId], rule: { ruleId: "RVC_BLANK_IMAGE_CONTAINERS" }, safeRecommendation: capture.hostIntegration.blankImageContainers > 0 ? "Repair the governed image source or rendering transport before approval." : null }));
    findings.push(finding({ findingCode: `${capture.viewportClass}_UNEXPECTED_ELEMENT_COLLISION`, category: "LAYOUT", state: capture.hostIntegration.unexpectedElementCollision ? "BLOCKED" : "PASS", summary: capture.hostIntegration.unexpectedElementCollision ? "Non-hero text or CTA controls overlap." : "No unexpected non-hero text or CTA collision was measured.", evidenceReferences: [capture.captureId], rule: { ruleId: "RVC_UNEXPECTED_ELEMENT_COLLISION" }, safeRecommendation: capture.hostIntegration.unexpectedElementCollision ? "Repair the component grid or flow so text and controls have distinct rendered bounds." : null }));
    findings.push(finding({ findingCode: `${capture.viewportClass}_CTA_COLLISION`, category: "LAYOUT", state: capture.hostIntegration.ctaHeadingCollision || capture.hostIntegration.ctaCopyCollision || capture.hostIntegration.ctaButtonCollision ? "BLOCKED" : "PASS", summary: capture.hostIntegration.ctaHeadingCollision || capture.hostIntegration.ctaCopyCollision || capture.hostIntegration.ctaButtonCollision ? "The final CTA heading, copy, or button overlaps another CTA element." : "Final CTA heading, copy, and button bounds are distinct.", evidenceReferences: [capture.captureId], rule: { ruleId: "RVC_CTA_COLLISION" }, safeRecommendation: capture.hostIntegration.ctaHeadingCollision || capture.hostIntegration.ctaCopyCollision || capture.hostIntegration.ctaButtonCollision ? "Use explicit CTA rows or groups with measured gaps." : null }));
    findings.push(finding({ findingCode: `${capture.viewportClass}_CTA_BUTTON_CONTAINER_OVERFLOW`, category: "OVERFLOW", state: capture.hostIntegration.buttonContainerOverflow ? "BLOCKED" : "PASS", summary: capture.hostIntegration.buttonContainerOverflow ? "The final CTA button exceeds its component container." : "The final CTA button is contained within its component.", evidenceReferences: [capture.captureId], rule: { ruleId: "RVC_CTA_BUTTON_CONTAINER_OVERFLOW" }, safeRecommendation: capture.hostIntegration.buttonContainerOverflow ? "Constrain the CTA button to its parent on this viewport." : null }));
    findings.push(finding({ findingCode: `${capture.viewportClass}_STICKY_HEADER_OCCLUSION`, category: "LAYOUT", state: capture.hostIntegration.stickyHeaderUnexpectedOcclusion ? "BLOCKED" : "PASS", summary: capture.hostIntegration.stickyHeaderUnexpectedOcclusion ? "A fixed or sticky header occludes primary content." : "No fixed or sticky header occlusion was measured.", evidenceReferences: [capture.captureId], rule: { ruleId: "RVC_STICKY_HEADER_OCCLUSION" }, safeRecommendation: capture.hostIntegration.stickyHeaderUnexpectedOcclusion ? "Repair host offsets or header positioning before approval." : null }));
    const planningGridPresent = capture.hostIntegration.planningGridPresent ?? true;
    findings.push(finding({ findingCode: `${capture.viewportClass}_PLANNING_GRID_BALANCE`, category: "LAYOUT", state: !planningGridPresent ? "NOT_EVALUATED" : capture.hostIntegration.planningGridBalanced ? "PASS" : "BLOCKED", summary: !planningGridPresent ? "Planning-grid semantics are absent in this governed capture." : capture.hostIntegration.planningGridBalanced ? "The seven-item planning grid is intentionally balanced." : "The planning grid leaves an unbalanced orphan position.", evidenceReferences: [capture.captureId], rule: { ruleId: "RVC_PLANNING_GRID_BALANCE" }, safeRecommendation: !planningGridPresent || capture.hostIntegration.planningGridBalanced ? null : "Center or span the final planning row without adding synthetic content." }));
