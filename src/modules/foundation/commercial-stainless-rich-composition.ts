...
export type CommercialStainlessRichPreview = {
  ...
  geometry: {
    ...
    proposedCanvasWidth: number;
    currentHeaderToContentGap: number;
    proposedHeaderToHeroGap: number;
    breakpoints: number[];
    fullPageHeights: { currentDesktop: number; proposedDesktop: number; currentMobile: number; proposedMobile: number; proposed1024: number; proposed768: number };
  };
};

export type CommercialStainlessPublicShell = {
  headHtml: string;
...
export function extractCommercialStainlessPublicShell(publicHtml: string): CommercialStainlessPublicShell {
  const headHtml = publicHtml.match(/<head[^>]*>([\s\S]*?)<\/head>/i)?.[1] ?? "";
  const bodyAttributes = publicHtml.match(/<body([^>]*)>/i)?.[1] ?? "";
  const headerHtml = publicHtml.match(/<header class=["']wp-block-template-part["'][^>]*>[\s\S]*?<\/header>/i)?.[0] ?? "";
  const footerMatches = [...publicHtml.matchAll(/<footer class=["']wp-block-template-part["'][^>]*>[\s\S]*?<\/footer>/gi)];
  const footerHtml = footerMatches.at(-1)?.[0] ?? "";
  if (!headHtml || !headerHtml || !footerHtml) throw new Error("COMMERCIAL_STAINLESS_PUBLIC_SHELL_REQUIRED");
  return { headHtml, bodyAttributes, headerHtml, footerHtml };
}
...
export function renderDesignBuildRichComposition(input: { page: SiteGeneratedPageRevision; inventory: CommercialStainlessPageInventoryItem[]; publicShell: CommercialStainlessPublicShell }): CommercialStainlessRichPreview {
  ...
  const preservedHead = input.publicShell.headHtml
    .replace(/<title>[\s\S]*?<\/title>/i, `<title>${escapeHtml(input.page.seoTitle)}</title>`)
    .replace(/<meta[^>]+name=["']description["'][^>]*>/i, `<meta name="description" content="${escapeHtml(input.page.metaDescription)}">`)
    .replace(/<link[^>]+rel=["']canonical["'][^>]*>/i, `<link rel="canonical" href="${escapeHtml(proof.currentCanonical)}">`);
  const html = `<!doctype html><html lang="en"><head>${preservedHead}<base href="${COMMERCIAL_STAINLESS_ORIGIN}/"><style>
...
@media(max-width:520px){.csc-shell-wrap,.csc-wrap{width:min(100% - 28px,1240px)}.csc-site-title{font-size:22px}.csc-global-nav{display:none}.csc-mobile-menu{display:block;position:relative;margin-top:18px}.csc-mobile-menu summary{padding:13px 16px;border:1px solid #bbb;font-size:12px;font-weight:900;list-style:none;text-align:center;text-transform:uppercase}.csc-mobile-menu summary::-webkit-details-marker{display:none}.csc-mobile-menu nav{display:grid;gap:1px;margin-top:8px;background:#ddd}.csc-mobile-menu a{padding:13px 14px;background:#fff;font-size:11px;font-weight:800;text-transform:uppercase}.csc-mobile-menu .csc-nav-cta{text-align:center}.csc-hero{min-height:620px}.csc-hero h1{font-size:42px}.csc-hero p{font-size:17px}.csc-actions{display:grid}.csc-benefits .csc-wrap,.csc-card-grid,.csc-process,.csc-industry-grid,.csc-input-list{grid-template-columns:1fr}.csc-benefit{border-right:0;border-bottom:1px solid #34383a}.csc-section h2,.csc-split h2,.csc-final-cta h2{font-size:38px}.csc-card{min-height:310px}.csc-split .csc-media{min-height:320px}.csc-split-copy{padding:48px 24px}.csc-inputs{gap:32px}}
  ...
  const completeHtml = html
    .replace("<body>", `<body${input.publicShell.bodyAttributes}><div class="wp-site-blocks">`)
    .replace(/<header class="csc-global-header">[\s\S]*?<\/header>(?=<main)/, input.publicShell.headerHtml)
    .replace(/<footer class="csc-global-footer">[\s\S]*?<\/footer><\/body>/, `${input.publicShell.footerHtml}</div></body>`)
    .replace("<strong>Commercial Applications</strong>", "<strong>Commercial Grade</strong>")
    .replace("Solutions organized around documented operating requirements.", "Commercial applications remain grounded in documented requirements.")
    .replace("<strong>Requirement-Led</strong>", "<strong>Nationwide Service</strong>")
    .replace("Open decisions remain explicit before finalization.", "Project location and coordination requirements remain part of scope review.")
    .replace("This preview does not add engineering, certification, delivery, or nationwide-service claims.", "This preview does not add engineering, certification, or delivery claims.");
  ...
  return { version: COMMERCIAL_STAINLESS_RICH_COMPOSITION_VERSION, previewOnly: true, wordpressMutation: false, publicationMutation: false, campaignMutation: false, page: proof, html: completeHtml, seoHashBefore: hash, seoHashAfter: hash, preservedLinks, media, geometry: { currentCanvasWidth: 645, proposedCanvasWidth: 1240, currentHeaderToContentGap: 785.19, proposedHeaderToHeroGap: 0, breakpoints: [1440, 1024, 768, 375], fullPageHeights: { currentDesktop: 5863, proposedDesktop: 10906, currentMobile: 7270, proposedMobile: 12683, proposed1024: 6186, proposed768: 6763 } } };
}
