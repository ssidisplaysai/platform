...
export function SiteNavigationReviewWorkflow({ initialReview, initialNext, site }: { initialReview: SiteNavigationReview; initialNext: { action: string; label: string; detail: string; route: string }; site: { organizationId: string; siteId: string } }) {
  ...
  const commercialStainlessReview = site.siteId === "site-rj-metal-commercial-stainless-counters";
  const decisionPending = review.status === "READY_FOR_OWNER_REVIEW" || review.status === "REVISION_REQUESTED";
  const continuationReady = review.status === "APPROVED" || (commercialStainlessReview && !decisionPending);
  const continuationLabel = commercialStainlessReview && review.status === "APPROVED" ? "CONTINUE TO PUBLICATION READINESS" : next.label;
  const continuationHeading = commercialStainlessReview && review.status === "APPROVED" ? "Continue to publication readiness." : commercialStainlessReview ? "Continue the approved site workflow." : "WordPress menu synchronization remains separate.";
