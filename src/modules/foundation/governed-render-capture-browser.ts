...
async function geometry(page: Page, assignments: readonly CaptureMediaAssignment[]) {
  return page.evaluate((knownAssignments) => {
    ...
    const planningGridPass = planningCards.length === 7 && rowCount(planningCards) === (desktop ? 2 : innerWidth <= 420 ? 7 : 4);
    ...
    const finalCtaCompositionPass = ctaWidthAlignedWithPage && ctaTextBalance && ctaButtonProminent && !excessiveCtaWhitespace;
