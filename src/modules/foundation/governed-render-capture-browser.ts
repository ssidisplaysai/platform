...
async function geometry(page: Page, assignments: readonly CaptureMediaAssignment[]) {
  return page.evaluate((knownAssignments) => {
    ...
    const contentWrap = document.querySelector(".saw-product .saw-wrap"); const contentWrapBounds = box(contentWrap);
    const productSplit = document.querySelector(".saw-product .saw-split"); const productImageBounds = box(document.querySelector(".saw-product .saw-media-frame")); const productCopyBounds = box(document.querySelector(".saw-product .saw-copy"));
    const applicationCards = Array.from(document.querySelectorAll(".saw-grid article")).map(box).filter((item): item is NonNullable<typeof item> => Boolean(item));
    const planningGridElement = document.querySelector(".saw-plan-grid");
    const planningGridPresent = Boolean(planningGridElement);
    const planningCards = Array.from(document.querySelectorAll(".saw-plan-grid > div")).map(box).filter((item): item is NonNullable<typeof item> => Boolean(item));
    ...
    const finalPlanningRow = planningRows.at(-1) ?? [];
    const planningGridBounds = box(planningGridElement);
    const finalPlanningLeft = finalPlanningRow.length ? Math.min(...finalPlanningRow.map((item) => item.x)) : 0;
    const finalPlanningRight = finalPlanningRow.length ? Math.max(...finalPlanningRow.map((item) => item.x + item.width)) : 0;
    const planningGridBalanced = Boolean(planningGridBounds && planningCards.length === 7 && (desktop ? planningRows.length === 2 && planningRows[0].length === 4 && finalPlanningRow.length === 3 && Math.abs((finalPlanningLeft + finalPlanningRight) / 2 - (planningGridBounds.x + planningGridBounds.width / 2)) <= 3 : innerWidth <= 420 ? planningRows.length === 7 : finalPlanningRow.length === 1 && Math.abs(finalPlanningRow[0].width - planningGridBounds.width) <= 3));
    ...
    const ctaButtonCollision = intersects(ctaButton, ctaHeading) || intersects(ctaButton, ctaCopy);
    const buttonContainerOverflow = Boolean(ctaButtonBounds && ctaInnerBounds && (ctaButtonBounds.x < ctaInnerBounds.x - 1 || ctaButtonBounds.y < ctaInnerBounds.y - 1 || ctaButtonBounds.x + ctaButtonBounds.width > ctaInnerBounds.x + ctaInnerBounds.width + 1 || ctaButtonBounds.y + ctaButtonBounds.height > ctaInnerBounds.y + ctaInnerBounds.height + 1));
    const collisionElements = Array.from(document.querySelectorAll(".saw-page section:not(.saw-hero) h2,.saw-page section:not(.saw-hero) h3,.saw-page section:not(.saw-hero) p,.saw-page section:not(.saw-hero) .saw-button")).filter(visible);
    const unexpectedElementCollision = collisionElements.some((left, index) => collisionElements.slice(index + 1).some((right) => !left.contains(right) && !right.contains(left) && left.closest("section") === right.closest("section") && intersects(left, right)));
    const headerPosition = header ? getComputedStyle(header).position : "";
    const stickyHeaderUnexpectedOcclusion = Boolean(header && ["fixed", "sticky"].includes(headerPosition) && intersects(header, hero ?? primary));
    const finalCtaCompositionPass = ctaWidthAlignedWithPage && ctaTextBalance && ctaButtonProminent && !excessiveCtaWhitespace && !ctaHeadingCollision && !ctaCopyCollision && !ctaButtonCollision && !buttonContainerOverflow;
    const rhythmSections = [document.querySelector(".saw-hero"), document.querySelector(".saw-product"), document.querySelector(".saw-applications"), document.querySelector(".saw-planning"), ctaSection].map(box).filter((item): item is NonNullable<typeof item> => Boolean(item));
    const sectionRhythmPass = rhythmSections.length === 5 && rhythmSections.every((item) => item.height >= 240 && item.height <= (desktop ? 1000 : 2400));
    ...
    return {
      ...
      hostIntegration: { headerOverlap: intersects(header, primary), footerOverlap: intersects(footer, primary), blankImageContainers, headerRegression: !(visible(header) && headerActions.length > 0 && headerActions.every(contained)), contentWidthBalanced, sectionRhythmPass, productIntroductionCompositionPass, applicationGridPass, planningGridPass, ctaWidthAlignedWithPage, ctaTextBalance, ctaButtonProminent, excessiveCtaWhitespace, ctaHeadingCollision, ctaCopyCollision, ctaButtonCollision, buttonContainerOverflow, unexpectedElementCollision, stickyHeaderUnexpectedOcclusion, planningGridPresent, planningGridBalanced, finalCtaCompositionPass },
      media, sections,
