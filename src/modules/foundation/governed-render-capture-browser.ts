...
async function geometry(page: Page, assignments: readonly CaptureMediaAssignment[]) {
  return page.evaluate((knownAssignments) => {
    ...
    const textRoot = document.querySelector("[data-genesis-primary-content]") ?? primary; const candidates = Array.from(textRoot.querySelectorAll("h1,h2,h3,p,a,small,figcaption")).filter((element) => visible(element) && Boolean(element.textContent?.trim()) && !element.querySelector("h1,h2,h3,p,a,small,figcaption")); const backgroundAwareTextContrast = candidates.map((element, index) => { const role = roleOf(element); const section = element.closest("section,[data-section-id]"); return { observationId: `${section?.id || section?.getAttribute("data-media-role") || "section"}-${role}-${index}`, viewport: String(innerWidth), sectionId: section?.id || section?.getAttribute("data-media-role") || "unscoped", textExcerpt: (element.textContent ?? "").replace(/\s+/g, " ").trim().slice(0, 240), ...backgroundOf(element), textRole: role, foregroundColor: getComputedStyle(element).color, requiredContrast: ["H1", "H2", "H3", "CAPTION", "LABEL"].includes(role) ? 3 : 4.5, source: document.querySelector("[data-native-wordpress-equivalent]") ? "HOST_RENDER" as const : "COMPOSITION_RENDER" as const }; });
