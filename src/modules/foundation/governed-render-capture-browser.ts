...
async function geometry(page: Page, assignments: readonly CaptureMediaAssignment[]) {
  return page.evaluate((knownAssignments) => {
    ...
    return {
      ...
      media, sections,
      themeIntegration: { visibleH1Count: visibleH1s.length, visibleH1Texts: visibleH1s.map((item) => item.textContent?.trim() ?? "").filter(Boolean), duplicateThemeTitleVisible: visible(document.querySelector(".page-title.the-title")), duplicateThemeFeaturedMediaVisible: visible(document.querySelector(".post-media.single-image")), globalHeaderPresent: visible(header), globalFooterPresent: visible(document.querySelector("footer,[role=contentinfo],#cafe-site-footer")), fontAuthorityExpected, headerActionsContained: headerActions.length > 0 && headerActions.every(contained), quoteCtaContained: quoteCtas.length > 0 && quoteCtas.every(contained), richComposition, overflowElements },
    };
  }, assignments);
}
