...
export async function captureGovernedRenderedPage(input: GovernedBrowserCaptureInput): Promise<GovernedBrowserCaptureResult> {
  ...
  try {
    ...
    const renderedContentHash = hashRenderedVisualContent(await page.content()); const screenshotHash = hashRenderedVisualContent(bytes); const screenshotComputedStyleCorrelationId = hashRenderedVisualContent(`${input.captureId}:${renderedContentHash}:${screenshotHash}:${styleSettlement.signature}`);
