  ...
  return (
    <section id="site-intelligence-workspace" className="space-y-6">
      ...
      {!workspace ? (
        ...
        </section><BinaryUploadForm props={props} workspace={workspace} onUploaded={setWorkspace} /></>
      ) : (
        <>
          ...
          <ResearchExecutionList workspace={workspace} />
          ...
          <CreativeInputForm busy={busy} onAction={action} />
          <BinaryUploadForm props={props} workspace={workspace} onUploaded={setWorkspace} />
          <AssetList props={props} workspace={workspace} />
...
function OpportunityBoard({ workspace, busy, onAction }: { workspace: Workspace; busy: boolean; onAction(body: Record<string, unknown>): void }) {
...
function OpportunityRow({ opportunity, busy, onAction }: { opportunity: SiteOpportunity; busy: boolean; onAction(body: Record<string, unknown>): void }) {
  const [capabilityEvidence, setCapabilityEvidence] = useState("");
  const [capabilityNotes, setCapabilityNotes] = useState("");
  const decide = (decision: OpportunityDecision) => onAction({ action: decision === "RESEARCH_MORE" ? "RESEARCH_MORE" : "DECIDE_OPPORTUNITY", opportunityId: opportunity.opportunityId, decision, reason: `Owner decision: ${decision}.` });
  const capability = (state: CapabilityEvidenceState) => onAction({ action: "VALIDATE_CAPABILITY", opportunityId: opportunity.opportunityId, state, evidenceIds: capabilityEvidence.trim() ? [capabilityEvidence.trim()] : [], notes: capabilityNotes, reason: `Owner capability decision: ${state}.` });
  return <article className="border-b border-zinc-800 p-5 last:border-0"><div className="flex flex-wrap justify-between gap-3"><div><h3 className="font-semibold text-white">{opportunity.name}</h3><p className="mt-1 text-xs text-zinc-500">{opportunity.category} · {opportunity.buyer} · confidence {opportunity.confidence}</p></div><span className="text-xs text-amber-300">{opportunity.capabilityState}</span></div><p className="mt-3 text-sm text-zinc-300">{opportunity.rationale}</p><p className="mt-2 text-xs text-zinc-500">Evidence: {opportunity.evidenceIds.join(", ") || "None"} · Competitors: {opportunity.competitorEntities.join(", ") || "None"}</p><div className="mt-4 flex flex-wrap gap-2">{(["APPROVED", "RESEARCH_MORE", "HOLD", "REJECTED"] as OpportunityDecision[]).map((decision) => <button key={decision} type="button" disabled={busy} onClick={() => decide(decision)} className="border border-zinc-700 px-3 py-2 text-xs text-zinc-200 disabled:opacity-40">{decision.replace("_", " ")}</button>)}</div><div className="mt-4 grid gap-2 sm:grid-cols-2"><label className="text-xs text-zinc-400">Capability evidence reference<input value={capabilityEvidence} onChange={(event) => setCapabilityEvidence(event.target.value)} className="mt-1 h-9 w-full border border-zinc-700 bg-zinc-900 px-2 text-white" /></label><label className="text-xs text-zinc-400">Owner notes<input value={capabilityNotes} onChange={(event) => setCapabilityNotes(event.target.value)} className="mt-1 h-9 w-full border border-zinc-700 bg-zinc-900 px-2 text-white" /></label></div><div className="mt-2 flex flex-wrap gap-2">{(["VERIFIED", "QUALIFIED", "REJECTED", "FUTURE_CAPABILITY"] as CapabilityEvidenceState[]).map((state) => <button key={state} type="button" disabled={busy || ((state === "VERIFIED" || state === "QUALIFIED") && !capabilityEvidence.trim())} onClick={() => capability(state)} className="border border-zinc-700 px-3 py-2 text-xs text-zinc-200 disabled:opacity-40">{state.replace("_", " ")}</button>)}</div></article>;
}

function CreativeInputForm({ busy, onAction }: { busy: boolean; onAction(body: Record<string, unknown>): void }) {
...
function BinaryUploadForm({ props, workspace, onUploaded }: { props: Props; workspace: Workspace | null; onUploaded(workspace: Workspace): void }) {
...
function AssetList({ props, workspace }: { props: Props; workspace: Workspace }) {
...
function ResearchExecutionList({ workspace }: { workspace: Workspace }) {
