  ...
  useEffect(() => {
    ...
    fetch(`/api/sites/${encodeURIComponent(props.siteId)}/intelligence`, {
      ...
      .then(async (response) => {
        ...
        if (!cancelled) setWorkspace(payload.workspace ?? null);
      })
    ...
    <section id="site-intelligence-workspace" className="space-y-6">
      ...
      {!workspace ? (
        <section className="border border-red-900 bg-red-950/20 p-6">
          ...
          <button type="button" disabled={busy} onClick={() => action({ action: "START", publicBrandIdentity: props.publicBrandIdentity, providerReference: "provider-bounded-site-research-v1", reason: "Owner explicitly started site intelligence." })} className="mt-5 bg-red-600 px-5 py-3 text-sm font-semibold text-white disabled:opacity-40">{busy ? "Starting…" : "START SITE INTELLIGENCE"}</button>
        </section>
      ) : (
        <>
...
function OpportunityRow({ opportunity, busy, onAction }: { opportunity: SiteOpportunity; busy: boolean; onAction(body: Record<string, unknown>): void }) {
  ...
  const decide = (decision: OpportunityDecision) => onAction({ action: "DECIDE_OPPORTUNITY", opportunityId: opportunity.opportunityId, decision, reason: `Owner decision: ${decision}.` });
