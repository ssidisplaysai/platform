...
function PostCapabilityNextStep({ workspace, busy, onAction }: { workspace: Workspace; busy: boolean; onAction(body: Record<string, unknown>): Promise<Workspace | null> }) {
  ...
  async function refreshStrategy() {
    const updated = await onAction({ action: "REFRESH_STRATEGY", reason: "Owner requested an updated strategy after final capability review." });
    if (!updated) return;
    requestAnimationFrame(() => {
      const review = document.getElementById("strategy-review");
...
function StrategyPanel({ workspace, busy, onAction }: { workspace: Workspace; busy: boolean; onAction(body: Record<string, unknown>): Promise<Workspace | null> }) {
  ...
  return <section className="border border-zinc-800 bg-zinc-950 p-5"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="font-semibold text-white">Site Strategy</h2><strong className="text-xs text-zinc-300">{workspace.strategyState}</strong></div>{proposal.status === "PROPOSED" ? <><div className="mt-4 border border-red-700 bg-red-950/30 p-5"><p className="text-xs font-semibold uppercase text-red-300">Updated strategy ready for review</p><h3 className="mt-2 text-xl font-semibold text-white">Current revision: {proposal.revision}</h3><p className="mt-1 text-sm font-semibold text-amber-200">Status: READY FOR REVIEW</p><p className="mt-3 text-sm text-zinc-300">Revision {proposal.revision} was generated from your finalized market and capability decisions. Review the strategy below, then approve it or request changes.</p><StrategyDecisionControls busy={busy} placement="top" onAction={onAction} /></div><StrategyRevisionHistory revisions={workspace.strategyRevisions} activeRevision={proposal.revision} /><RichStrategyReview proposal={proposal} /><StrategyDecisionControls busy={busy} placement="bottom" onAction={onAction} /></> : <><StrategyRevisionHistory revisions={workspace.strategyRevisions} activeRevision={proposal.revision} /><RichStrategyReview proposal={proposal} />{proposal.status === "APPROVED" ? <div className="mt-4"><p className="text-xs text-emerald-300">This revision is approved and read-only.</p><button type="button" disabled={busy} onClick={() => onAction({ action: "REOPEN_STRATEGY", reason: "Owner indicated strategy approval was unintentional; reopened for review before further strategy work." })} className="mt-3 border border-amber-700 px-3 py-2 text-xs font-semibold text-amber-200 disabled:opacity-40">REOPEN FOR REVIEW</button></div> : <StrategyRevisionEditor proposal={proposal} busy={busy} onAction={onAction} />}</>}</section>;
}

function StrategyDecisionControls({ busy, placement, onAction }: { busy: boolean; placement: "top" | "bottom"; onAction(body: Record<string, unknown>): Promise<Workspace | null> }) {
  const decisions = [{ decision: "APPROVED", label: "APPROVE STRATEGY" }, { decision: "REVISION_REQUESTED", label: "REQUEST CHANGES" }, { decision: "REJECTED", label: "REJECT" }] as const;
  return <div aria-label={`${placement} strategy review actions`} className={`${placement === "top" ? "mt-4" : "sticky bottom-0 mt-6 border-t border-zinc-800 bg-zinc-950/95 py-4"} flex flex-wrap gap-2`}>{decisions.map(({ decision, label }) => <button key={decision} type="button" disabled={busy} onClick={() => onAction({ action: "DECIDE_STRATEGY", decision, reason: `Owner strategy decision: ${decision}.` })} className={decision === "APPROVED" ? "bg-emerald-700 px-4 py-3 text-sm font-semibold text-white disabled:opacity-40" : "border border-zinc-700 px-4 py-3 text-sm font-semibold text-zinc-200 disabled:opacity-40"}>{label}</button>)}</div>;
}

function StrategyRevisionEditor({ busy, onAction }: { proposal: SiteStrategyProposal; busy: boolean; onAction(body: Record<string, unknown>): Promise<Workspace | null> }) {
  const [revisionInstructions, setRevisionInstructions] = useState("");
  return <div className="mt-4 border border-amber-800 bg-amber-950/20 p-5"><h3 className="font-semibold text-amber-100">What should Genesis change?</h3><p className="mt-1 text-sm text-zinc-300">Describe the requested strategy changes in plain language. The current revision remains in history.</p><textarea value={revisionInstructions} onChange={(event) => setRevisionInstructions(event.target.value)} rows={4} className="mt-3 w-full border border-amber-800 bg-zinc-950 p-3 text-white" placeholder="Describe the changes you want Genesis to make." /><button type="button" disabled={busy || !revisionInstructions.trim()} onClick={() => onAction({ action: "GENERATE_REVISED_STRATEGY", revisionInstructions, reason: `Owner requested a revised strategy: ${revisionInstructions.trim()}` })} className="mt-3 bg-red-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-40">GENERATE REVISED STRATEGY</button></div>;
}

export function LegacyStrategyRevisionEditor({ proposal, busy, onAction }: { proposal: SiteStrategyProposal; busy: boolean; onAction(body: Record<string, unknown>): Promise<Workspace | null> }) {
...
function StrategyRevisionHistory({ revisions, activeRevision }: { revisions: SiteStrategyProposal[]; activeRevision: number }) { return <div className="mt-4 border border-zinc-800 p-4"><h3 className="text-xs font-semibold uppercase text-zinc-400">Revision History</h3><div className="mt-2 space-y-2">{revisions.map((revision) => { const active = revision.revision === activeRevision; return <div key={revision.revision} className={`flex flex-wrap justify-between gap-2 border p-3 text-xs ${active ? "border-red-700 bg-red-950/20" : "border-zinc-900 bg-zinc-950"}`}><span className={active ? "font-semibold text-red-200" : "text-zinc-400"}>Revision {revision.revision} · {active && revision.status === "PROPOSED" ? "CURRENTLY AWAITING OWNER DECISION" : revision.status}</span><span className="text-zinc-500">{active ? "Active review" : "Historical revision"} · Created {revision.createdAt} by {revision.createdBy}{revision.decidedAt ? ` · decided ${revision.decidedAt} by ${revision.decidedBy}` : ""}</span></div>; })}</div></div>; }
