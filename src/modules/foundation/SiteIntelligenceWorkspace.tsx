...
function OpportunityRow({ opportunity, capabilityEvidenceOptions, busy, onAction }: { opportunity: SiteOpportunity; capabilityEvidenceOptions: CapabilityEvidenceOption[]; busy: boolean; onAction(body: Record<string, unknown>): Promise<Workspace | null> }) {
  ...
  async function capability(state: CapabilityEvidenceState) {
    ...
    const result = await onAction({ action: "VALIDATE_CAPABILITY", opportunityId: opportunity.opportunityId, state, evidenceIds: capabilityEvidence, notes: capabilityNotes, reason: `Owner capability decision: ${state}.` });
  ...
  return <article className="border-b border-zinc-800 p-5 last:border-0">
    ...
    <fieldset className="mt-5 border-t border-zinc-800 pt-4"><legend className="text-xs font-semibold uppercase text-zinc-400">Capability Authority</legend><p className="mt-1 text-xs text-zinc-500">Verified and Qualified require selected owner-controlled evidence. Market approval remains independent.</p><div className="mt-3 grid gap-3"><div><p className="text-xs text-zinc-400">Supporting evidence</p>{capabilityEvidenceOptions.length ? <div className="mt-2 grid gap-2 sm:grid-cols-2">{capabilityEvidenceOptions.map((option) => <label key={option.referenceId} className="flex gap-2 border border-zinc-800 p-3 text-xs text-zinc-300"><input type="checkbox" checked={capabilityEvidence.includes(option.referenceId)} onChange={(event) => setCapabilityEvidence((current) => event.target.checked ? [...new Set([...current, option.referenceId])] : current.filter((id) => id !== option.referenceId))} /><span><strong className="block text-zinc-100">{option.label}</strong><span className="block text-zinc-500">{option.sourceType} · {option.classification ?? "Owner authority"} · {option.createdAt}</span><span className="block text-zinc-500">{option.notes ?? option.provenance}</span></span></label>)}</div> : <p className="mt-2 text-xs text-amber-300">Add an owner-supplied URL or upload to the Reference Library before verifying or qualifying capability.</p>}</div><label className="text-xs text-zinc-400">Owner notes<input value={capabilityNotes} onChange={(event) => setCapabilityNotes(event.target.value)} className="mt-1 h-9 w-full border border-zinc-700 bg-zinc-900 px-2 text-white" /></label></div><p className="mt-2 text-xs text-amber-300">Supporting evidence is required for Verified or Qualified capability. Qualification notes should describe limits or conditions.</p><div className="mt-2 flex flex-wrap gap-2">{(["VERIFIED", "QUALIFIED", "REJECTED", "FUTURE_CAPABILITY"] as CapabilityEvidenceState[]).map((state) => { const selected = opportunity.capabilityState === state; return <button key={state} type="button" aria-pressed={selected} disabled={busy || selected} onClick={() => capability(state)} className={`border px-3 py-2 text-xs font-semibold disabled:cursor-not-allowed ${selected ? "border-emerald-600 bg-emerald-950 text-emerald-200" : "border-zinc-700 text-zinc-200 disabled:opacity-60"}`}>{state.replaceAll("_", " ")}</button>; })}</div></fieldset>
...
function StrategyPanel({ workspace, busy, onAction }: { workspace: Workspace; busy: boolean; onAction(body: Record<string, unknown>): Promise<Workspace | null> }) {
  ...
  const verified = workspace.opportunities.filter((opportunity) => opportunity.capabilityState === "VERIFIED").length;
  const qualified = workspace.opportunities.filter((opportunity) => opportunity.capabilityState === "QUALIFIED").length;
...
function RichStrategyReview({ proposal }: { proposal: SiteStrategyProposal }) {
  const sections: Array<{ label: string; values: string[] }> = [{ label: "Audiences", values: [proposal.primaryAudience, ...proposal.secondaryAudiences] }, { label: "Market / Vertical Priorities", values: proposal.majorVerticals }, { label: "Product / Service Families", values: proposal.productServiceFamilies }, { label: "Information Architecture", values: proposal.informationArchitecture }, { label: "Sitemap", values: proposal.proposedSitemap }, { label: "Conversion Paths", values: proposal.conversionPaths }, { label: "CTA Hierarchy", values: proposal.ctaHierarchy }, { label: "Proof / Trust Requirements", values: proposal.trustProofRequirements }, { label: "Required Product Authority", values: proposal.proposedProductAuthority }, { label: "Homepage Goals", values: proposal.homepageGoals }];
