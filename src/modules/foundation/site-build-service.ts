...
export function getSiteBuildWorkspace(site: SiteConfiguration) {
  ...
  const next = {
    ...
    SITE_VISUAL_REVIEW: { action: "REVIEW_SITE_DESIGNS", label: "REVIEW SITE DESIGNS", detail: "Review the page-specific visual assemblies before site QA or publication." },
  ...
  const visualStage = stage === "WORDPRESS_DRAFT_REVIEW" && currentHomeVisualAssembly?.status === "READY_FOR_OWNER_REVIEW" ? "HOME_DESIGN_REVIEW" : stage === "WORDPRESS_DRAFT_REVIEW" && remainingVisualSummary.assembled === remainingVisualSummary.expected && remainingVisualSummary.expected > 0 ? "SITE_VISUAL_REVIEW" : stage;
  const visualNext = visualStage === "HOME_DESIGN_REVIEW" ? { action: "REVIEW_DESIGNED_HOME", label: "REVIEW DESIGNED HOME", detail: "Review the rendered Home visual assembly before any design is propagated to the remaining pages." } : visualStage === "SITE_VISUAL_REVIEW" ? { action: "REVIEW_SITE_DESIGNS", label: "REVIEW SITE DESIGNS", detail: `Review ${remainingVisualSummary.assembled} page-specific visual assemblies before site QA or publication.` } : next;
  return { site: { organizationId: site.organizationId, siteId: site.siteId, displayName: site.displayName }, generation, session, ...records, imageCandidates, visualAssemblies, currentVisualAssemblies, currentVisualAssembly, currentHomeVisualAssembly, remainingVisualAssemblies, remainingVisualSummary, pageReview, stale, stage: visualStage, next: visualNext, publication: { state: site.publishingStatus, enabled: site.enabled } };
}
