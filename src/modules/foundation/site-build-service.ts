...
export function getSiteBuildWorkspace(site: SiteConfiguration) {
  ...
  const visualStage: SiteBuildStage = visualContinuation?.stage ?? stage;
  const visualNext = visualContinuation ? { action: visualContinuation.action, label: visualContinuation.label, detail: visualContinuation.detail } : next;
  const nextRoute = visualContinuation ? `/sites/${site.siteId}/build/${visualContinuation.path}` : visualStage === "WORDPRESS_DRAFT_REVIEW" ? `/sites/${site.siteId}/build/wordpress-review` : `/sites/${site.siteId}/build`;
