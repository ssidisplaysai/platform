...
export function getSiteBuildWorkspace(site: SiteConfiguration) {
  ...
  const downstreamStage: SiteBuildStage | null = currentNavigationReview ? currentNavigationReview.status === "APPROVED" ? "PUBLICATION_READINESS" : currentNavigationReview.status === "PUBLICATION_READINESS_CONFIRMED" || currentNavigationReview.status === "PUBLICATION_AUTHORIZED" ? "PUBLICATION_AUTHORIZATION" : "NAVIGATION_REVIEW" : null;
...
function nextForStage(stage: "NAVIGATION_REVIEW" | "WORDPRESS_MENU_SYNC" | "PUBLICATION_READINESS" | "PUBLICATION_AUTHORIZATION") {
  return stage === "NAVIGATION_REVIEW" ? { action: "REVIEW_NAVIGATION", label: "REVIEW NAVIGATION", detail: "Review and explicitly decide the approved site architecture's header and footer navigation proposal." } : stage === "WORDPRESS_MENU_SYNC" ? { action: "SYNC_WORDPRESS_MENU", label: "SYNC WORDPRESS MENU", detail: "Apply the approved navigation to WordPress through a separate explicit mutation and verification gate." } : stage === "PUBLICATION_READINESS" ? { action: "REVIEW_PUBLICATION_READINESS", label: "REVIEW PUBLICATION READINESS", detail: "Verify the complete site, navigation, and publication safeguards before requesting authorization." } : { action: "AUTHORIZE_PUBLICATION", label: "REVIEW PUBLICATION AUTHORIZATION", detail: "Explicit owner publication authorization remains required. Nothing is published automatically." };
}
