...
export function authorizeSitePublicationPolicy(site: SiteConfiguration) { const workspace = getSiteBuildWorkspace(site); if (!workspace.currentNavigationReview || workspace.currentNavigationReview.status !== "PUBLICATION_AUTHORIZED" || workspace.stage !== "PUBLICATION_EXECUTION_REVIEW" || site.enabled || site.publishingStatus !== "disabled" || site.lifecycleState !== "configuring") throw new Error("PUBLICATION_POLICY_AUTHORIZATION_BOUNDARY_NOT_READY"); const updated = updateSite(site.siteId, { publicationPolicy: "publish_after_gates" }); if (!updated.validation.valid || !updated.site || updated.site.enabled || updated.site.publishingStatus !== "disabled" || updated.site.lifecycleState !== "configuring") throw new Error("PUBLICATION_POLICY_TRANSITION_FAILED"); return updated.site; }
