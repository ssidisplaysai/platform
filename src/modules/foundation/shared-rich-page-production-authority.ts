...
export type SharedRichPageProductionProfile = {
  ...
  presentation: { authority: string; family: string } | null;
...
export const SHARED_RICH_PAGE_AUTHORITY_MANIFEST: Readonly<Record<SharedRichPageCapability, SharedAuthorityBinding>> = {
  ...
  CLAIM_CANONICALIZATION: { authorityName: "Zero Authority Claim Canonicalization", implementationPath: "src/modules/glw/zero-authority-claim-canonicalization.ts", owningModuleOrService: "rehabilitateGlwExistingArtifact", contractOrPolicy: "GLW_ZERO_AUTHORITY_CLAIM_CANONICALIZATION_V2_1", consumers: ["GLW reference pipeline", "GLW existing-artifact recovery"], reusableBy: ["GLW campaigns", "existing GLW generated and published content"], state: "SHARED" },
...
export function richPageHostIntegrationCss(profile: SharedRichPageProductionProfile): string {
  ...
  const immersive = profile.presentation?.authority === "LED_DISPLAY_WAREHOUSE_PRESENTATION_V1" || profile.presentation?.authority === "PROJECTOR_ENCLOSURE_PRESENTATION_V1" ? ".genesis-rich-page-host>article>.entry-content,.genesis-rich-page-host>article>.post-content,.genesis-rich-page-host .wp-block-post-content{width:100%!important;max-width:none!important;margin-inline:0!important;padding-inline:0!important}" : "";
  return `html,body.genesis-rich-page-host{max-width:100%;overflow-x:clip!important}.genesis-rich-page-host>header{position:relative!important;inset:auto!important;width:100%!important;max-width:100%!important;transform:none!important}.genesis-rich-page-host>article{position:relative;z-index:1;width:100%;max-width:100%;margin:0!important;overflow:clip}.genesis-rich-page-host>footer{position:relative!important;clear:both}${title}${featured}${immersive}@media(max-width:${profile.layout.mobileBreakpoint}px){.genesis-rich-page-host header .elementor-nav-menu--main{display:none!important}.genesis-rich-page-host header .elementor-nav-menu--dropdown[aria-hidden="true"]{display:none!important;max-width:100%!important}.genesis-rich-page-host header .elementor-menu-toggle{display:flex!important}}`;
}

export function nativeTitlePolicyMatches(hostPolicy: SharedRichPageProductionProfile["host"], hideTitle: unknown): boolean {
  const normalizedHideTitle = typeof hideTitle === "string" ? hideTitle.trim().toLowerCase() : "";
  return hostPolicy.suppressNativeTitle
    ? normalizedHideTitle === "yes"
    : normalizedHideTitle !== "yes";
}
