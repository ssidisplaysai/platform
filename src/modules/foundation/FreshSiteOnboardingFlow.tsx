...
export function FreshSiteOnboardingFlow() {
  ...
  const identityReady = intent === "fresh" && Boolean(siteName.trim() && organizationId && cleanDomain);
  ...
  return (
    <section id="fresh-site-onboarding" className="overflow-hidden rounded-lg border border-zinc-800 bg-zinc-950">
      ...
      <div className="p-5 sm:p-6">
        {step === 1 ? (
          <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="text-sm text-zinc-300">Site Name<input required value={siteName} onChange={(event) => { setSiteName(event.target.value); setPreflight(null); }} placeholder="Example Fresh Site" className="mt-1 h-11 w-full border border-zinc-700 bg-zinc-900 px-3 text-white outline-none focus:border-red-500" /></label>
              <label className="text-sm text-zinc-300">Organization<select value={organizationId} onChange={(event) => { setOrganizationId(event.target.value); setPreflight(null); }} className="mt-1 h-11 w-full border border-zinc-700 bg-zinc-900 px-3 text-white">{organizations.map((organization) => <option key={organization.id} value={organization.id}>{organization.name}</option>)}</select></label>
              <label className="text-sm text-zinc-300 md:col-span-2">Domain<input required value={domain} onChange={(event) => { setDomain(event.target.value); setPreflight(null); }} placeholder="example.com" inputMode="url" className="mt-1 h-11 w-full border border-zinc-700 bg-zinc-900 px-3 text-white outline-none focus:border-red-500" /></label>
            </div>
        ...
        {step === 4 && assessment ? (
          <div className="space-y-6">
            <div><h3 className="text-lg font-semibold text-white">Readiness and integration assessment</h3><p className="mt-1 text-sm text-zinc-400">Canonical readiness remains conservative while this site is disabled for generation.</p></div>
            <div className="grid gap-4 lg:grid-cols-2">
              <section className="border border-zinc-800 p-4"><h4 className="font-semibold text-white">Integration Assessment</h4><p className="mt-3 text-2xl font-semibold text-emerald-300">{assessment.complexity}</p><p className="mt-1 text-sm text-zinc-400">Estimated Genesis onboarding effort: {assessment.effort}</p></section>
              <section className="border border-zinc-800 p-4"><h4 className="font-semibold text-white">WordPress Inventory</h4><dl className="mt-3 grid grid-cols-2 gap-2 text-sm text-zinc-300"><div>Pages: {assessment.inventory.pages}</div><div>Posts: {assessment.inventory.posts}</div><div>Media: {assessment.inventory.media}</div><div>Products: {assessment.inventory.productCount}</div></dl><p className="mt-3 text-xs text-zinc-500">Empty content and media are valid for a fresh site.</p></section>
            </div>
            <section className="border border-zinc-800"><div className="border-b border-zinc-800 px-4 py-3 font-semibold text-white">Integration requirements</div>{assessment.requirements.map((requirement) => <div key={requirement.key} className="grid gap-1 border-b border-zinc-900 px-4 py-3 last:border-0 sm:grid-cols-[11rem_7rem_1fr]"><span className="text-sm text-zinc-200">{requirement.label}</span><span className={`text-xs font-semibold ${requirement.status === "READY" || requirement.status === "NOT_REQUIRED" ? "text-emerald-300" : requirement.status === "MISSING" ? "text-red-300" : "text-amber-300"}`}>{requirement.status}</span><span className="text-xs text-zinc-500">{requirement.details}</span></div>)}</section>
            ...
            <div className="flex justify-end"><button type="button" onClick={() => setStep(5)} className="bg-red-600 px-4 py-2 text-sm font-semibold text-white">Complete Onboarding</button></div>
          </div>
        ) : null}
