...
import { COMMERCIAL_STAINLESS_RICH_COMPOSITION_VERSION, classifyCommercialStainlessPage, createCommercialStainlessRolloutPlan, inventoryCommercialStainlessPages, renderDesignBuildRichComposition } from "../commercial-stainless-rich-composition";
...
describe("Commercial Stainless rich composition preview", () => {
  ...
  const preview = renderDesignBuildRichComposition({ page: pages.find((page: { canonicalPath: string }) => page.canonicalPath === "/design-build-fabrication/") as never, inventory });
  ...
  test("renders only Design-Build as a wide reusable preview with one global shell", () => {
    ...
    expect(preview.geometry).toEqual({ currentCanvasWidth: 645, proposedCanvasWidth: 1240, breakpoints: [1440, 1024, 768, 375] });
    expect(preview.html.match(/<header\b/g)).toHaveLength(1);
    expect(preview.html.match(/<footer\b/g)).toHaveLength(1);
    expect(preview.html).toContain('<details class="csc-mobile-menu">');
