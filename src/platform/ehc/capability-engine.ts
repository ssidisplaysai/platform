  ...
  return {
    buildAdvertisement(declared, available) {
      ...
      return {
        ...
        availableCapabilities: statuses.filter((status) => status.availability === "AVAILABLE").map((status) => status.capability),
        unavailableCapabilities: statuses.filter((status) => status.availability === "UNAVAILABLE").map((status) => status.capability),
        statuses,
      };
    },
  };
