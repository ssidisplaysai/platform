...
console.log('═══════════════════════════════════════════════════════════════════════════════');
console.log('        PROCESSING ZACH\'S REAL DISCOVERY INTERVIEW - GENESIS ENGINE');
console.log('═══════════════════════════════════════════════════════════════════════════════\n');

console.log(`Input: ZACH_ANDERSON_DISCOVERY_INTERVIEW.txt`);
console.log(`Size: ${interviewContent.length.toLocaleString()} characters\n`);

// Create document manually with proper structure
const sourceId = 'src_zach_real_discovery_interview';
const document = {
  sourceId,
...
for (const line of lines) {
  ...
  if (trimmed.match(/^[A-Z][A-Z\s—]+$/) && trimmed.length > 10) {
    ...
    currentBlock = { type: 'paragraph', text: '', raw: '' };
  } else if (trimmed.match(/^What|^Is there|^How|^List|^Walk me|^Who|^On a|^Are there/) && trimmed.endsWith('?')) {
    if (currentBlock.text.trim()) {
      blocks.push({
        type: currentBlock.type,
        text: currentBlock.text.trim(),
...
console.log(`✓ Zach_Real.document.json (${docJson.content.length.toLocaleString()} bytes)`);
...
console.log(`✓ Zach_Real.interview.json (${intJson.content.length.toLocaleString()} bytes)`);
...
console.log(`✓ Zach_Real.result.json (${resultJson.content.length.toLocaleString()} bytes)`);
