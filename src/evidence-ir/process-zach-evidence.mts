/**
 * Process Zach's Discovery Interview through Evidence IR Compiler (Stage 2)
 *
 * This script:
 * 1. Loads Zach's real Discovery Interview (from Stage 1)
...
async function main() {
  ...
  console.log('EVIDENCE IR STAGE 2 - ZACH INTERVIEW PROCESSING');
  console.log('='.repeat(70));
  console.log();
  ...
  try {
    ...
    const zachPath = path.join(discoveryOutputDir, 'Zach_Real.interview.json');
    const evidenceOutputDir = path.join(workspaceRoot, 'evidence-output');

    console.log('Configuration:');
    console.log(`  Discovery Input: ${zachPath}`);
    console.log(`  Evidence Output: ${evidenceOutputDir}`);
    console.log();

    // Check input file exists
    try {
      await fs.access(zachPath);
    } catch {
      console.error(`✗ Discovery file not found: ${zachPath}`);
    ...
    // Load discovery
    const discovery = await pipeline.loadDiscoveryJSON(zachPath);
