/**
 * Verify Policy Renderer Determinism
 *
 * Regenerate all 7 entities twice and verify all
 * policy markdown files are byte-for-byte identical.
 */
...
// From tools/genesis/verify-policy-determinism.mjs, go up 2 levels to platform/
...
function computeHashes(basePath) {
  ...
  for (const entity of entities) {
    const policyPath = join(basePath, entity, `${entity}.policies.md`);
    try {
...
async function verify() {
  console.log('🧪 Policy Renderer Determinism Verification');
  ...
  console.log(`✓ Generated ${count1} policy documents`);
  ...
  console.log(`✓ Generated ${count2} policy documents`);
  ...
  if (different === 0) {
    console.log('✅ Policy documents are deterministic!');
    process.exit(0);
  } else {
    console.log('❌ Policy documents are NOT deterministic');
    process.exit(1);
  }
}
