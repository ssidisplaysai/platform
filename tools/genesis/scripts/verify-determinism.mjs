...
console.log('\n🔍 Verifying EventsRenderer Determinism...\n');

// First generation
console.log('Generation 1...');
await generate();
const hash1 = await getFileHashes(path.join(projectRoot, 'out/generated/entities'), 'Events.ts');
...
await generate();
const hash2 = await getFileHashes(path.join(projectRoot, 'out/generated/entities'), 'Events.ts');
...
for (const entity of entities) {
  const pattern = `${entity}Events.ts`;
...
if (allMatch) {
  console.log('\n✓ All Events.ts artifacts are byte-for-byte identical\n');
  process.exit(0);
} else {
